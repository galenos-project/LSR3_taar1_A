...
source("wrangling_u2/wrangling_functions.R", local = TRUE) # unchanged relative to u1

LSR <- 'LSR3'

# Import SyRF outcome data
LSR3_SyRFOutcomes <- read_csv("data_u2/Quantitative_data_-_2025_08_03_-_c494b1ae-4cf4-4618-b91a-e69a2b815bdd_-_Investigators_Unblinded.csv")
...
# Fix bad pivot for disease model
reconciled_cohort_label <- reconciled_cohort_label %>%
  rename( `Category of disease model induction method` = `Category of disease model induction method:[1]`, 
          `Which type of control was used for disease model?` = `Which type of control was used for disease model?[1]`, 
          `Type of pharmacological induction` = `Type of pharmacological induction:[1]`, 
          `Dose of pharmacological induction (dose)` = `Dose of pharmacological induction (dose)[1]`, 
          `Dose of pharmacological induction (units)` = `Dose of pharmacological induction (units)[1]`,
          `Type of neurodev induction`= `Type of neurodev induction:[1]`, 
          `Timing of disease model induction:` = `Timing of disease model induction:[1]`, 
          `Route of administration (pharmacological):` = `Route of administration (pharmacological):[1]`, 
          `Duration/intensity` = `Duration/intensity[1]`, 
          `Gene manipulated:` = `Gene manipulated:[1]`,
          `Type of manipulation:` = `Type of manipulation:[1]`, 
          `IsDiseaseModelControl` = `IsDiseaseModelControl[1]`, 
          `ModelID` = `ModelID[1]`, 
          `DiseaseModelLabel(s)` = `DiseaseModelLabel(s)[1]`) %>%
  select(-`Category of disease model induction method:[2]`, -`Which type of control was used for disease model?[2]`, -`Type of pharmacological induction:[2]`, -`Dose of pharmacological induction (dose)[2]`, -`Dose of pharmacological induction (units)[2]`, 
         -`Type of neurodev induction:[2]`, -`Timing of disease model induction:[2]`, -`Route of administration (pharmacological):[2]`, -`Duration/intensity[2]`, -`Gene manipulated:[2]`, -`Type of manipulation:[2]`, -`IsDiseaseModelControl[2]`, -`ModelID[2]`, -`DiseaseModelLabel(s)[2]`)
...
dataall <- dataall %>% 
  mutate(OutcomeType = case_when(str_detect(OutcomeLabel_I, regex("lma|locomotion|loc|locomotor|oft|horizontal|distance|vertical|climbing", ignore_case = TRUE)) ~ "Locomotor activity",
...
df <- df %>% 
    mutate(CategoryDiseaseInduction = case_when(`Category of disease model induction method_I` == "Pharmacological (e.g. psychostimulants, NMDA antagonists)" & `Type of pharmacological induction_I` == "cocaine" ~ "Psychostimulant - Cocaine", 
                                                `Category of disease model induction method_I` == "Pharmacological (e.g. psychostimulants, NMDA antagonists)" & `Type of pharmacological induction_I` == "MK-801" ~ "Psychostimulant - MK-801",
                                                `Category of disease model induction method_I` == "Pharmacological (e.g. psychostimulants, NMDA antagonists)" & `Type of pharmacological induction_I` == "phencyclidine" ~ "Psychostimulant - PCP", 
                                                `Category of disease model induction method_I` == "Pharmacological (e.g. psychostimulants, NMDA antagonists)" & `Type of pharmacological induction_I` == "ketamine" ~ "Psychostimulant - Ketamine", 
                                                `Category of disease model induction method_I` == "Pharmacological (e.g. psychostimulants, NMDA antagonists)" & `Type of pharmacological induction_I` == "apomorphine" ~ "Psychostimulant - APO", 
                                                `Category of disease model induction method_I` == "Pharmacological (e.g. psychostimulants, NMDA antagonists)" & `Type of pharmacological induction_I` == "Other" ~ "Psychostimulant - Other", 
                                                `Category of disease model induction method_I` == "Pharmacological (e.g. psychostimulants, NMDA antagonists)" & `Type of pharmacological induction_I` == "other" ~ "Psychostimulant - Other", 
                                                `Category of disease model induction method_I` == "Pharmacological (e.g. psychostimulants, NMDA antagonists)" & `Type of pharmacological induction_I` == "amphetamine" ~ "Psychostimulant - Amphetamine", 
                                                `Category of disease model induction method_I` == "Genetic (e.g. DISC1 KO, DAT KO, D2R overexpression)" & `Gene manipulated:_I` == "DAT" ~ "Genetic - DAT KO", 
                                                `Category of disease model induction method_I` == "Genetic (e.g. DISC1 KO, DAT KO, D2R overexpression)" & `Gene manipulated:_I` == "DAT KO" ~ "Genetic - DAT KO", 
                                                TRUE ~ `Category of disease model induction method_I`))  # Keep other values unchanged))
...
df <- df %>% 
  ...
  mutate(DurationOfTreatmentWeeks = case_when(`Duration of treatment[1]` == 999 ~ NA_real_,
                                              `Duration of treatment[1]` == 998 ~ NA_real_,
                                              `Duration of treatment[1]` == 9999 ~ NA_real_,
                                              `Duration of treatment[1]` == 1000 ~ NA_real_,
                                              `Duration of treatment[1]` == 1001 ~ NA_real_,
                                              `Unit of measurement for treatment duration[1]` == "Days" ~ `Duration of treatment[1]`/7, 
...
df <- df %>% 
  ...
  mutate(DurationOfTreatmentWeeks = case_when(`Duration of treatment[2]` == 999 ~ NA_real_,
                                              `Duration of treatment[2]` == 998 ~ NA_real_,
                                              `Duration of treatment[2]` == 9999 ~ NA_real_,
                                              `Duration of treatment[2]` == 1000 ~ NA_real_,
                                              `Duration of treatment[2]` == 1001 ~ NA_real_,
                                              `Unit of measurement for treatment duration[2]` == "Days" ~ `Duration of treatment[2]`/7, 
...
df <- df %>% 
  mutate(pE50 = case_when(DrugName == "LK000764" ~ 8.40, 
                          ...
                          #DrugName == "Compound 8b" ~ 7.9, # edited based on information from Spiros
  ...
  mutate(EC50mM = case_when(DrugName == "LK000764" ~ 0.004, 
                            ...
                            DrugName == "Compound 8b" ~ NA, # edited based on information given by Spiros
                            DrugName == "ZH8659" ~ NA, 
