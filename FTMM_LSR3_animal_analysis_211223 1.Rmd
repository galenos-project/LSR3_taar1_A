...
# Set working directory for all subsequent chunks
# knitr::opts_knit$set(root.dir = "~/OneDrive - University of Edinburgh/Applications/WT Mental Health/LSR3/FRFT files 241223")
...
source("util 5 211223.R")

# obtain the data and prepare them for analysis - all data cleaning routines should be in this .R script 
source("wrangling_functions 201223.R", local = TRUE)
source("data_wrangle_script 201223.R")
...
The flow of study selection is shown in Figure 1 (*to be included*). Studies included were published between `r min(article_metadata$Year)` and `r max(article_metadata$Year)`. Overall, this analysis includes `r length(table(df$StudyId))` studies containing `r nrow(df)` comparisons.
...
Included studies used `r df %>% select(all_of(c("InterventionID[1]", "InterventionID[2]"))) %>% distinct() %>% nrow()` unique intervention/treatment procedures.

### TAAR1 specific descriptives

`r nrow(df%>%filter(CohortType == "Simple intervention"))` comparisons tested the effects of a single intervention, `r nrow(df%>%filter(CohortType == "Combination intervention"))`  comparisons tested the effects of a combination of multiple interventions (typically a TAAR1 agonist and a licensed antipsychotic), and `r nrow(df%>%filter(CohortType == "Intervention and TAAR1KO comb."))` comparisons tested the effects of an intervention in TAAR1 knockout animals.

# Multilevel meta-analysis
In the field of pre-clinical research, it is common for studies to provide data that can be used to calculate multiple effect sizes. Effect sizes from the same study are not statistically independent, and therefore cannot be analysed using standard meta-analytic techniques. We use a multilevel meta-analytic model to account for this non-independence.

Multilevel meta-analytic models pool effect sizes which are not statistically independent (i.e., effect sizes are correlated) at multiple hierarchical (nested) levels ([Harrer et al., 2021](https://bookdown.org/MathiasHarrer/Doing_Meta_Analysis_in_R/)).

The multilevel meta-analytic model in this analysis was built considering the following hierarchical levels as random effects:
...
### Effect sizes
The scales/units used to measure outcomes in preclinical studies often differ between studies. Therefore, the primary effect size used for meta-analysis of preclinical studies is the standardised mean difference (SMD, Hedge’s g). Where the performance of untreated animals can be known or inferred, we will perform a sensitivity analysis using normalised mean difference (NMD). 

## Experiment types 

Within the literature we identified 3 distinct categories of experiments: 

1.  Experiments investigating the effect of administering a TAAR1 agonist alone. We refer to these as **Simple interventions**.

2.  Experiments investigating the effect of administering a TAAR1 agonist alongside a currently licensed anti-psychotic. We refer to these as **Combination interventions**

3.  Experiments investigating the effect of administering a TAAR1 agonist alongside TAAR1 antagonism (here administered to TAAR1 knock-out animals, i.e. genetic TAAR1 antagonism). We refer to these as **TAAR1 antagonism experiments**. 
...
Of the `r length(table(df_S$StudyId))` included studies using simple interventions, the number of studies and individual effect sizes for each outcome were:
...
Outcomes marked with a * represent primary outcomes. All others are secondary outcomes

Outcomes with ≥2 independent effect sizes are proceeded to meta-analysis. In this iteration of the review, this includes `r df %>% group_by(outcome_type) %>% filter(n_distinct(StudyId) > 1) %>% summarise(n = n_distinct(StudyId)) %>% arrange(desc(n)) %>% pull(outcome_type) %>% unique() %>% tolower() %>% { if (length(.) > 1) paste(paste(head(., -1), collapse = ", "), "and", tail(., 1)) else .}`. 
...
##### 2. Category of disease induction
...
##### 3. Route of intervention administration
...
##### 4. Prophylactic or therapeutic intervention 
...
##### 6. The intervention administered 
...
##### 7. The efficacy of the drug (i.e. whether the drug is a partial or full agonist)
...
#### Heterogeneity explained by covariates (Simple interventions, Locomotor activity)
...
| Covariate | \( \tau^2 \) |
|---|:---:|
| *Sex* | `r sum(SMD_S_LMA_sex$subgroup_analysis$sigma2)` |
| *Category of disease induction* | `r sum(SMD_S_LMA_CatDisInd$subgroup_analysis$sigma2)` |
| *Route of intervention administration* | `r sum(SMD_S_LMA_AdminRoute$subgroup_analysis$sigma2)` |
| *Prophylactic or therapeutic intervention* | `r sum(SMD_S_LMA_ProphThera$subgroup_analysis$sigma2)` |
| *The intervention administered* | `r sum(SMD_S_LMA_Drug$subgroup_analysis$sigma2)` |
| *Drug efficacy* | `r sum(SMD_S_LMA_DrugEfficacy$subgroup_analysis$sigma2)` |
| *Drug selectivity* | `r sum(SMD_S_LMA_DrugSelectivity$subgroup_analysis$sigma2)` |
| Unadjusted results | `r sum(SMD_S_LMA$sigma2)` |


### Subgroup analyses for Cognition 
...
- The dose of the intervention administered (where there is sufficient data)
...
# Reporting quality : ARRIVE checklist
```{r warning=FALSE, eval = TRUE, echo = FALSE}
ARRIVE_summary(df)
```
...
# Risks of Bias : SyRCLE Risk of Bias Tool
```{r warning=FALSE, eval = TRUE, echo = FALSE}
