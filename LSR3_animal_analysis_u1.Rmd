...
author: "Francesca Tinsdeall, Fiona Ramage, Virginia Chiocchia, Spyridon Siafis and Malcolm Macleod"
...
source("wrangling_u1/data_wrangle_script.R")
...
df$StudyId <- df$StudyId_I
df$Title <- df$Title_I
df$Year <- df$Year_I
df$Authors <- df$Authors_I
df$DrugLabel_C <- df$DrugLabel1_C
...
prisma_data <- read_csv("data_u1/LSR3_prisma_190825.csv")
...
df3 <- df3 %>% 
  mutate(TAAR1RS = ifelse(df3$SortLabel == "TvC_KO", "TAAR1 R KO", "WT"))

trow <- merge(df3, RoB, by = 'StudyId')
tab1 <- trow[,c(16,5,4,14,8,13,15)]
tab2 <- tab1 %>%
  group_by(Study, CategoryDiseaseInduction, Strain, CompLable, outcome_type, TAAR1RS) %>%
  summarise(subjects_sum = sum(N))


colnames(tab2) <- c("Study", "Model", "Strain", "Comparison", "Outcome","TAAR1 Receptor Status", "N")
...
tab2 <- read_csv("data_u1/tab_formatted.csv")
tab2 <- tab2 %>%
  mutate(across(where(is.character), ~ str_remove_all(., fixed('"'))))
tab2 <- tab2 %>%
  mutate(across(where(is.character), ~ replace_na(., "")))
#tab2$N <- as.numeric(tab2$N)
original_data <- tab2[1:91, 1:7]


# Rows to have bold lines beneath
rows_with_bold_lines <- c(1,3,4,9,10,11,13,14,15,21,27,30,31,40,44,51,52,53,54,55,65,80,81,83)
# Generate HTML table with adjusted styles

generate_html_table <- function(data, bold_rows) {
  cat('<div style="text-align: center;">')
  cat('<table style="width: 100%; border-collapse: collapse;">')
  
  # Header
  cat('<tr style="border-bottom: 2px solid black;">')
  cat(paste0('<th style="padding: 8px; text-align: center;">', 
             names(data), '</th>', collapse = ''))
  cat('</tr>')
  ...
  for (i in seq_len(nrow(data))) {
    ...
    for (j in seq_len(ncol(data))) {
      ...
      border <- if (i %in% bold_rows || i == nrow(data)) {
        'border-bottom: 2px solid black;'
      } else {
        'border-bottom: 1px solid black;'
      }
      
      align <- if (j == 1) 'text-align: left;' else 'text-align: center;'
      style <- paste0('padding: 8px; ', border, align)
      
      cat(paste0('<td style="', style, '">', data[i, j], '</td>'))
    }
    cat('</tr>')
  }
...
# Usage
generate_html_table(original_data, rows_with_bold_lines)


```

References of included studies are located in the appendix. Included studies used `r nrow(unique(df[,c(435,436,437,61:72)]))` unique disease model induction procedures.\*some animals contributed to more than one experimental outcome; this number reflects the number of outcomes rather than the number of animals.
...
-   Pre pulse inhibition\*: `r length(table(df_S_PPI$StudyId))` studies and `r nrow(df_S_PPI)` comparisons in `r {strains <- unique(df_S_PPI$Strain); if (length(strains) > 1) paste(length(strains), "strains") else paste(length(strains), "strain")}`
...
```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, fig.height=25, fig.width=18}
...
```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide', fig.width=20, fig.height=4}
SMD_S_LMA_PharmDisInd <- hierarchical_subgroup_analysis(df, "TvC", "Locomotor activity", 0.5)
```

```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
forest_hierarchical(SMD_S_LMA_PharmDisInd$plotdata, "Locomotor Activity")
```

```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
SMD_S_LMA_PharmDisInd_noI <- subgroup_SMD_pharm(df, "TvC", "Locomotor activity", "CDI_level2", 0.5)
```

The p-value for the association between different categories of pharmacological induction ('Stimulants or dopamine agonists' versus 'NMDA antagonists') and outcome reported was `r if(round(SMD_S_LMA_PharmDisInd_noI$QMp,3)>0.001){paste0('= ',round(SMD_S_LMA_PharmDisInd_noI$QMp,3))}else{'< 0.001'}`.

| Level | Number of categories for that level included in this analysis | Attributable variance |
|:----------------------:|:----------------------:|:----------------------:|
| Strain | `r ifelse('Strain' %in% SMD_S_LMA_PharmDisInd$analyses$global$s.names, SMD_S_LMA_PharmDisInd$analyses$global$s.nlevels[[which(SMD_S_LMA_PharmDisInd$analyses$global$s.names == 'Strain')]], 0)` | `r ifelse('Strain' %in% SMD_S_LMA_PharmDisInd$analyses$global$s.names, SMD_S_LMA_PharmDisInd$analyses$global$sigma2[[which(SMD_S_LMA_PharmDisInd$analyses$global$s.names == 'Strain')]], NA)` |
| Study x Strain | `r ifelse('Strain/StudyId' %in% SMD_S_LMA_PharmDisInd$analyses$global$s.names, SMD_S_LMA_PharmDisInd$analyses$global$s.nlevels[[which(SMD_S_LMA_PharmDisInd$analyses$global$s.names == 'Strain/StudyId')]], 0)` | `r ifelse('Strain/StudyId' %in% SMD_S_LMA_PharmDisInd$analyses$global$s.names, SMD_S_LMA_PharmDisInd$analyses$global$sigma2[[which(SMD_S_LMA_PharmDisInd$analyses$global$s.names == 'Strain/StudyId')]], NA)` |
| Study x Strain x Experiment | `r ifelse('Strain/StudyId/ExperimentID_I' %in% SMD_S_LMA_PharmDisInd$analyses$global$s.names, SMD_S_LMA_PharmDisInd$analyses$global$s.nlevels[[which(SMD_S_LMA_PharmDisInd$analyses$global$s.names == 'Strain/StudyId/ExperimentID_I')]], 0)` | `r ifelse('Strain/StudyId/ExperimentID_I' %in% SMD_S_LMA_PharmDisInd$analyses$global$s.names, SMD_S_LMA_PharmDisInd$analyses$global$sigma2[[which(SMD_S_LMA_PharmDisInd$analyses$global$s.names == 'Strain/StudyId/ExperimentID_I')]], 'NA')` |
...
The p-value for the association between whether the drug was highly selective, or also manifests 5-HT1A ("5HT1A part. ag") or $/a$-2 adrenergic ("Adr a2 ag") effects, and outcome reported was `r round(SMD_S_LMA_DrugSelectivity_noI$QMp,3)`.
...
The estimate for $\beta$ was `r SMD_S_LMA_RO5263397_dose$beta[2]` (p = `r  round(SMD_S_LMA_RO5263397_dose$pval[2],3)`).

| Level | Number of categories for that level included in this analysis | Attributable variance |
|:----------------------:|:----------------------:|:----------------------:|
| Strain | `r ifelse('Strain' %in% SMD_S_LMA_RO5263397_dose$s.names, SMD_S_LMA_RO5263397_dose$s.nlevels[[which(SMD_S_LMA_RO5263397_dose$s.names == 'Strain')]], 0)` | `r ifelse('Strain' %in% SMD_S_LMA_RO5263397_dose$s.names, SMD_S_LMA_RO5263397_dose$sigma2[[which(SMD_S_LMA_RO5263397_dose$s.names == 'Strain')]], NA)` |
| Study x Strain | `r ifelse('Strain/StudyId' %in% SMD_S_LMA_RO5263397_dose$s.names, SMD_S_LMA_RO5263397_dose$s.nlevels[[which(SMD_S_LMA_RO5263397_dose$s.names == 'Strain/StudyId')]], 0)` | `r ifelse('Strain/StudyId' %in% SMD_S_LMA_RO5263397_dose$s.names, SMD_S_LMA_RO5263397_dose$sigma2[[which(SMD_S_LMA_RO5263397_dose$s.names == 'Strain/StudyId')]], NA)` |
| Study x Strain x Experiment | `r ifelse('Strain/StudyId/ExperimentID_I' %in% SMD_S_LMA_RO5263397_dose$s.names, SMD_S_LMA_RO5263397_dose$s.nlevels[[which(SMD_S_LMA_RO5263397_dose$s.names == 'Strain/StudyId/ExperimentID_I')]], 0)` | `r ifelse('Strain/StudyId/ExperimentID_I' %in% SMD_S_LMA_RO5263397_dose$s.names, SMD_S_LMA_RO5263397_dose$sigma2[[which(SMD_S_LMA_RO5263397_dose$s.names == 'Strain/StudyId/ExperimentID_I')]], 'NA')` |
...
The estimate for $\beta$ was `r SMD_S_LMA_RO5203648_dose$beta[2]` (p = `r  round(SMD_S_LMA_RO5203648_dose$pval[2],3)`).

| Level | Number of categories for that level included in this analysis | Attributable variance |
|:----------------------:|:----------------------:|:----------------------:|
| Strain | `r ifelse('Strain' %in% SMD_S_LMA_RO5203648_dose$s.names, SMD_S_LMA_RO5203648_dose$s.nlevels[[which(SMD_S_LMA_RO5203648_dose$s.names == 'Strain')]], 0)` | `r ifelse('Strain' %in% SMD_S_LMA_RO5203648_dose$s.names, SMD_S_LMA_RO5203648_dose$sigma2[[which(SMD_S_LMA_RO5203648_dose$s.names == 'Strain')]], NA)` |
| Study x Strain | `r ifelse('Strain/StudyId' %in% SMD_S_LMA_RO5203648_dose$s.names, SMD_S_LMA_RO5203648_dose$s.nlevels[[which(SMD_S_LMA_RO5203648_dose$s.names == 'Strain/StudyId')]], 0)` | `r ifelse('Strain/StudyId' %in% SMD_S_LMA_RO5203648_dose$s.names, SMD_S_LMA_RO5203648_dose$sigma2[[which(SMD_S_LMA_RO5203648_dose$s.names == 'Strain/StudyId')]], NA)` |
| Study x Strain x Experiment | `r ifelse('Strain/StudyId/ExperimentID_I' %in% SMD_S_LMA_RO5203648_dose$s.names, SMD_S_LMA_RO5203648_dose$s.nlevels[[which(SMD_S_LMA_RO5203648_dose$s.names == 'Strain/StudyId/ExperimentID_I')]], 0)` | `r ifelse('Strain/StudyId/ExperimentID_I' %in% SMD_S_LMA_RO5203648_dose$s.names, SMD_S_LMA_RO5203648_dose$sigma2[[which(SMD_S_LMA_RO5203648_dose$s.names == 'Strain/StudyId/ExperimentID_I')]], 'NA')` |
...
```{r label = 'RO5166017 regression', message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
...
```{r label = 'LK000764 regression', message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
...
```{r label = 'RO5256390 regression', message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
...
```{r label = 'C50b regression', message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
...
```{r label = 'ZH8651 regression', message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
SMD_S_LMA_ZH8651_dose <- metaregression_analysis_by_drug(df, "TvC", "Locomotor activity", "ZH8651", "DoseOfIntervention_mgkg", 0.5)
```
...
filter(SortLabel == "TvC") %>%
filter(outcome_type == "Locomotor activity") %>%
filter(DrugName == "ZH8651") %>%
filter(!is.na(SMDv)) %>%
filter(!is.na(!!sym("DoseOfIntervention_mgkg")))
diag2 <- n_distinct(diag1$StudyId)
diag3 <- nrow(diag1)

output_text0 <- paste0("ZH8651: There were ", diag3, " comparisons from ", diag2, " publication(s).")

```

`r output_text0`

```{r label = 'ZH8659 regression', message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
SMD_S_LMA_ZH8659_dose <- metaregression_analysis_by_drug(df, "TvC", "Locomotor activity", "ZH8659", "DoseOfIntervention_mgkg", 0.5)
```
...
filter(SortLabel == "TvC") %>%
filter(outcome_type == "Locomotor activity") %>%
filter(DrugName == "ZH8659") %>%
filter(!is.na(SMDv)) %>%
filter(!is.na(!!sym("DoseOfIntervention_mgkg")))
diag2 <- n_distinct(diag1$StudyId)
diag3 <- nrow(diag1)

output_text0 <- paste0("ZH8659: There were ", diag3, " comparisons from ", diag2, " publication(s).")

```

`r output_text0`

```{r label = 'c50a regression', message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
...
```{r label = 'RO5073012 regression', message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
...
```{r label = '8b regression', message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
SMD_S_LMA_Compound8b_dose <- metaregression_analysis_by_drug(df, "TvC", "Locomotor activity", "Compound 8b", "DoseOfIntervention_mgkg", 0.5)
```
...
filter(SortLabel == "TvC") %>%
filter(outcome_type == "Locomotor activity") %>%
filter(DrugName == "Compound 8b") %>%
filter(!is.na(SMDv)) %>%
filter(!is.na(!!sym("DoseOfIntervention_mgkg")))
diag2 <- n_distinct(diag1$StudyId)
diag3 <- nrow(diag1)

output_text0 <- paste0("Compound 8b: There were ", diag3, " comparisons from ", diag2, " publication(s).")

```

`r output_text0`

```{r label = '6e regression', message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
SMD_S_LMA_Compound6e_dose <- metaregression_analysis_by_drug(df, "TvC", "Locomotor activity", "Compound 6e", "DoseOfIntervention_mgkg", 0.5)
```
...
filter(SortLabel == "TvC") %>%
filter(outcome_type == "Locomotor activity") %>%
filter(DrugName == "Compound 6e") %>%
filter(!is.na(SMDv)) %>%
filter(!is.na(!!sym("DoseOfIntervention_mgkg")))
diag2 <- n_distinct(diag1$StudyId)
diag3 <- nrow(diag1)

output_text0 <- paste0("Compound 6e: There were ", diag3, " comparisons from ", diag2, " publication(s).")

```

`r output_text0`

```{r label = '7b regression', message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
SMD_S_LMA_Compound7b_dose <- metaregression_analysis_by_drug(df, "TvC", "Locomotor activity", "Compound 7b", "DoseOfIntervention_mgkg", 0.5)
```
...
filter(SortLabel == "TvC") %>%
filter(outcome_type == "Locomotor activity") %>%
filter(DrugName == "Compound 7b") %>%
filter(!is.na(SMDv)) %>%
filter(!is.na(!!sym("DoseOfIntervention_mgkg")))
diag2 <- n_distinct(diag1$StudyId)
diag3 <- nrow(diag1)

output_text0 <- paste0("Compound 7b: There were ", diag3, " comparisons from ", diag2, " publication(s).")

```

`r output_text0`

```{r label = 'AP163 regression',  message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
...
```{r label = 'guanfacine regression', message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
SMD_S_LMA_Guanfacine_dose <- metaregression_analysis_by_drug(df, "TvC", "Locomotor activity", "Guanfacine", "DoseOfIntervention_mgkg", 0.5)
```
...
filter(SortLabel == "TvC") %>%
filter(outcome_type == "Locomotor activity") %>%
filter(DrugName == "Guanfacine") %>%
filter(!is.na(SMDv)) %>%
filter(!is.na(!!sym("DoseOfIntervention_mgkg")))
diag2 <- n_distinct(diag1$StudyId)
diag3 <- nrow(diag1)

output_text0 <- paste0("Guanfacine: There were ", diag3, " comparisons from ", diag2, " publication(s).")

```

`r output_text0`

```{r label = 'ZH8667 regression', message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
SMD_S_LMA_ZH8667_dose <- metaregression_analysis_by_drug(df, "TvC", "Locomotor activity", "ZH8667", "DoseOfIntervention_mgkg", 0.5)
```
...
filter(SortLabel == "TvC") %>%
filter(outcome_type == "Locomotor activity") %>%
filter(DrugName == "ZH8667") %>%
filter(!is.na(SMDv)) %>%
filter(!is.na(!!sym("DoseOfIntervention_mgkg")))
diag2 <- n_distinct(diag1$StudyId)
diag3 <- nrow(diag1)

output_text0 <- paste0("ZH8667: There were ", diag3, " comparisons from ", diag2, " publication(s).")
...
SMD_S_LMA_PharmDisIndI <- subgroup_SMDI_pharm(df, "TvC", "Locomotor activity", "CDI_level2", 0.5)
...
| \- | *Genetic* | `r SMD_S_LMA_CatDisIndI$beta[1]` | `r SMD_S_LMA_CatDisIndI$ci.lb[1]` to `r SMD_S_LMA_CatDisIndI$analysis$ci.ub[1]` | \- |
| \- | *Pharmacological* | `r SMD_S_LMA_CatDisIndI$beta[2]` | `r SMD_S_LMA_CatDisIndI$ci.lb[2]` to `r SMD_S_LMA_CatDisIndI$ci.ub[2]` | \- |
| Pharmacological class of model induction | \- | \- | \- | `r round((r2_ml(SMD_S_LMA_PharmDisIndI)[1]*100),1)`% |
| \- | *NMDA antagonist* | `r SMD_S_LMA_PharmDisIndI$beta[1]` | `r SMD_S_LMA_PharmDisIndI$ci.lb[1]` to `r SMD_S_LMA_PharmDisIndI$analysis$ci.ub[1]` | \- |
| \- | *Stimulant or Dopamine agonist* | `r SMD_S_LMA_PharmDisIndI$beta[2]` | `r SMD_S_LMA_PharmDisIndI$ci.lb[2]` to `r SMD_S_LMA_PharmDisIndI$ci.ub[2]` | \- |
...
| \- | *Subcutaneous* | `r SMD_S_LMA_AdminRouteI$beta[3]` | `r SMD_S_LMA_AdminRouteI$ci.lb[3]` to `r SMD_S_LMA_AdminRouteI$ci.ub[3]` | \- |
...
| \- | *Compound 50B* | `r SMD_S_LMA_DrugI$beta[3]` | `r SMD_S_LMA_DrugI$ci.lb[3]` to `r SMD_S_LMA_DrugI$ci.ub[3]` | \- |
| \- | *Compound 6e* | `r SMD_S_LMA_DrugI$beta[4]` | `r SMD_S_LMA_DrugI$ci.lb[4]` to `r SMD_S_LMA_DrugI$ci.ub[4]` | \- |
| \- | *Compound 7b* | `r SMD_S_LMA_DrugI$beta[5]` | `r SMD_S_LMA_DrugI$ci.lb[5]` to `r SMD_S_LMA_DrugI$ci.ub[5]` | \- |
| \- | *Compound 8b* | `r SMD_S_LMA_DrugI$beta[6]` | `r SMD_S_LMA_DrugI$ci.lb[6]` to `r SMD_S_LMA_DrugI$ci.ub[6]` | \- |
| \- | *Guanfacine* | `r SMD_S_LMA_DrugI$beta[7]` | `r SMD_S_LMA_DrugI$ci.lb[7]` to `r SMD_S_LMA_DrugI$ci.ub[7]` | \- |
| \- | *LK000764* | `r SMD_S_LMA_DrugI$beta[8]` | `r SMD_S_LMA_DrugI$ci.lb[8]` to `r SMD_S_LMA_DrugI$ci.ub[8]` | \- |
| \- | *RO5073012* | `r SMD_S_LMA_DrugI$beta[9]` | `r SMD_S_LMA_DrugI$ci.lb[9]` to `r SMD_S_LMA_DrugI$ci.ub[9]` | \- |
| \- | *RO5166017* | `r SMD_S_LMA_DrugI$beta[10]` | `r SMD_S_LMA_DrugI$ci.lb[10]` to `r SMD_S_LMA_DrugI$ci.ub[10]` |  |
| \- | *RO5203648* | `r SMD_S_LMA_DrugI$beta[11]` | `r SMD_S_LMA_DrugI$ci.lb[11]` to `r SMD_S_LMA_DrugI$ci.ub[11]` | \- |
| \- | *RO5256390* | `r SMD_S_LMA_DrugI$beta[12]` | `r SMD_S_LMA_DrugI$ci.lb[12]` to `r SMD_S_LMA_DrugI$ci.ub[12]` | \- |
| \- | *RO5263397* | `r SMD_S_LMA_DrugI$beta[13]` | `r SMD_S_LMA_DrugI$ci.lb[13]` to `r SMD_S_LMA_DrugI$ci.ub[13]` | \- |
| \- | *SEP-363856 (Ultaront)* | `r SMD_S_LMA_DrugI$beta[14]` | `r SMD_S_LMA_DrugI$ci.lb[14]` to `r SMD_S_LMA_DrugI$ci.ub[14]` | \- |
| \- | *ZH8651* | `r SMD_S_LMA_DrugI$beta[15]` | `r SMD_S_LMA_DrugI$ci.lb[15]` to `r SMD_S_LMA_DrugI$ci.ub[15]` | \- |
| \- | *ZH8659* | `r SMD_S_LMA_DrugI$beta[16]` | `r SMD_S_LMA_DrugI$ci.lb[16]` to `r SMD_S_LMA_DrugI$ci.ub[16]` | \- |
| \- | *ZH8667* | `r SMD_S_LMA_DrugI$beta[17]` | `r SMD_S_LMA_DrugI$ci.lb[17]` to `r SMD_S_LMA_DrugI$ci.ub[17]` | \- |
...
| \- | *Low - 5HT1A* | `r SMD_S_LMA_DrugSelectivityI$beta[2]` | `r SMD_S_LMA_DrugSelectivityI$ci.lb[2]` to `r SMD_S_LMA_DrugSelectivityI$ci.ub[2]` | \- |
| \- | *Low - a2 Adr* | `r SMD_S_LMA_DrugSelectivityI$beta[3]` | `r SMD_S_LMA_DrugSelectivityI$ci.lb[3]` to `r SMD_S_LMA_DrugSelectivityI$ci.ub[3]` |  |
| \- | *Unclear* | `r SMD_S_LMA_DrugSelectivityI$beta[4]` | `r SMD_S_LMA_DrugSelectivityI$ci.lb[4]` to `r SMD_S_LMA_DrugSelectivityI$ci.ub[4]` | \- |
...
For locomotor activity, `r df_S_LMA %>% filter(NMD_possible == "TRUE") %>% nrow()` out of `r nrow(df_S_LMA)` comparisons, i.e. `r round(((df_S_LMA %>% filter(NMD_possible == "TRUE") %>% nrow())/(nrow(df_S_LMA)))*100,1)` % of comparisons, had data available for a Sham group and for these studies it was possible to calculate an NMD estimate of effect size.
...
```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, fig.height=30, fig.width=12}
forest_metafor_NMD(NMD_S_LMA, "locomotor hyperactivity")
```

For TAAR1 Agonist v Control, TAAR1 interventions had a pooled effect on locomotor activity of NMD = `r round(NMD_S_LMA[["beta"]][1],1)` (95% CI: `r round(NMD_S_LMA[["ci.lb"]],1)` to `r round(NMD_S_LMA[["ci.ub"]],1)`) with a prediction interval of `r round(predict(NMD_S_LMA)$pi.lb,1)` to `r round(predict(NMD_S_LMA)$pi.ub,1)`). For reference the pooled effect size for SMD was `r SMD_S_LMA[["beta"]][1]` (95% CI: `r SMD_S_LMA[["ci.lb"]]` to `r SMD_S_LMA[["ci.ub"]]`).
...
```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, fig.height = 5}
...
```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
SMD_S_PPI <- run_ML_SMD(df, "TvC", "Prepulse inhibition", 0.5)
if (any(class(SMD_S_PPI) == "rma.mv")) {
  output_text1 <- "Figure 2.3.3 - Forest plot of effects on pre pulse inhibition for TAAR1 Agonist vs control"
} else {
  output_text1 <- ""
}
...
forest_metafor(SMD_S_PPI, "TvC", "Prepulse inhibition")
```

```{r echo=FALSE}
if (any(class(SMD_S_PPI) == "rma.mv")) {
  output_text3 <- paste0("For TAAR1 Agonist v Control, TAAR1 interventions had a pooled effect on pre pulse inhibition of SMD =", round(SMD_S_PPI[['beta']], 3), " (95% CI: ", round(SMD_S_PPI[['ci.lb']], 3), " to ", round(SMD_S_PPI[['ci.ub']], 3), ", with a prediction interval of ", round(predict(SMD_S_PPI)$pi.lb, 3), " to ", round(predict(SMD_S_PPI)$pi.ub, 3),").")
} else {
...
`r SMD_S_PPI[["k"]]` experimental comparisons were reported in `r length(unique(SMD_S_PPI$data$ExperimentID_I))` experiments reported from `r length(unique(SMD_S_PPI$data$StudyId))` publications and involving `r length(unique(SMD_S_PPI$data$Strain))` different animal strains.

| Level | Number of categories for that level included in this analysis | Attributable variance |
|:----------------------:|:----------------------:|:----------------------:|
| Strain | `r ifelse('Strain' %in% SMD_S_PPI$s.names, SMD_S_PPI$s.nlevels[[which(SMD_S_PPI$s.names == 'Strain')]], 0)` | `r ifelse('Strain' %in% SMD_S_PPI$s.names, SMD_S_PPI$sigma2[[which(SMD_S_PPI$s.names == 'Strain')]], NA)` |
| Study x Strain | `r ifelse('Strain/StudyId' %in% SMD_S_PPI$s.names, SMD_S_PPI$s.nlevels[[which(SMD_S_PPI$s.names == 'Strain/StudyId')]], 0)` | `r ifelse('Strain/StudyId' %in% SMD_S_PPI$s.names, SMD_S_PPI$sigma2[[which(SMD_S_PPI$s.names == 'Strain/StudyId')]], NA)` |
| Study x Strain x Experiment | `r ifelse('ExperimentID_I' %in% SMD_S_PPI$s.names, SMD_S_PPI$s.nlevels[[which(SMD_S_PPI$s.names == 'ExperimentID_I')]], 0)` | `r ifelse('ExperimentID_I' %in% SMD_S_PPI$s.names, SMD_S_PPI$sigma2[[which(SMD_S_PPI$s.names == 'ExperimentID_I')]], 'NA')` |
...
```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide', out.width = "100%"}
SMD_S_PPI_sex <- subgroup_analysis(df, "TvC", "Prepulse inhibition", "Sex", 0.5)
```

```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
  forest_subgroup(SMD_S_PPI_sex$plotdata, "Sex", "Prepulse inhibition","Sex")
```

```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
SMD_S_PPI_sex_noI <- subgroup_SMD(df, "TvC", "Prepulse inhibition", "Sex", 0.5)
```

The p-value for the association between the sex of animal groups used and outcome reported was `r if(round(SMD_S_PPI_sex_noI$QMp,3)>0.001){paste0('= ',round(SMD_S_PPI_sex_noI$QMp,3))}else{'< 0.001'}`.

| Level | Number of categories for that level included in this analysis | Attributable variance |
|:----------------------:|:----------------------:|:----------------------:|
| Strain | `r ifelse('Strain' %in% SMD_S_PPI_sex$analysis$s.names, SMD_S_PPI_sex$analysis$s.nlevels[[which(SMD_S_PPI_sex$analysis$s.names == 'Strain')]], 0)` | `r ifelse('Strain' %in% SMD_S_PPI_sex$analysis$s.names, SMD_S_PPI_sex$analysis$sigma2[[which(SMD_S_PPI_sex$analysis$s.names == 'Strain')]], NA)` |
| Study x Strain | `r ifelse('Strain/StudyId' %in% SMD_S_PPI_sex$analysis$s.names, SMD_S_PPI_sex$analysis$s.nlevels[[which(SMD_S_PPI_sex$analysis$s.names == 'Strain/StudyId')]], 0)` | `r ifelse('Strain/StudyId' %in% SMD_S_PPI_sex$analysis$s.names, SMD_S_PPI_sex$analysis$sigma2[[which(SMD_S_PPI_sex$analysis$s.names == 'Strain/StudyId')]], NA)` |
| Study x Strain x Experiment | `r ifelse('ExperimentID_I' %in% SMD_S_PPI_sex$analysis$s.names, SMD_S_PPI_sex$analysis$s.nlevels[[which(SMD_S_PPI_sex$analysis$s.names == 'ExperimentID_I')]], 0)` | `r ifelse('ExperimentID_I' %in% SMD_S_PPI_sex$analysis$s.names, SMD_S_PPI_sex$analysis$sigma2[[which(SMD_S_PPI_sex$analysis$s.names == 'ExperimentID_I')]], 'NA')` |
...
```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide', out.width = "100%"}
SMD_S_PPI_AdminRoute <- subgroup_analysis(df, "TvC", "Prepulse inhibition", "InterventionAdministrationRoute", 0.5)
```

```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
forest_subgroup(SMD_S_PPI_AdminRoute$plotdata, "InterventionAdministrationRoute", "Prepulse inhibition","Route of \nAdministration")
```

```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
SMD_S_PPI_AdminRoute_noI <- subgroup_SMD(df, "TvC", "Prepulse inhibition", "InterventionAdministrationRoute", 0.5)
```

The p-value for the association between whether genetic or pharmacological models were used and outcome reported was `r if(round(SMD_S_PPI_AdminRoute_noI$QMp,3)>0.001){paste0('= ',round(SMD_S_PPI_AdminRoute_noI$QMp,3))}else{'< 0.001'}`.

| Level | Number of categories for that level included in this analysis | Attributable variance |
|:----------------------:|:----------------------:|:----------------------:|
| Strain | `r ifelse('Strain' %in% SMD_S_PPI_AdminRoute$analysis$s.names, SMD_S_PPI_AdminRoute$analysis$s.nlevels[[which(SMD_S_PPI_AdminRoute$analysis$s.names == 'Strain')]], 0)` | `r ifelse('Strain' %in% SMD_S_PPI_AdminRoute$analysis$s.names, SMD_S_PPI_AdminRoute$analysis$sigma2[[which(SMD_S_PPI_AdminRoute$analysis$s.names == 'Strain')]], NA)` |
| Study x Strain | `r ifelse('Strain/StudyId' %in% SMD_S_PPI_AdminRoute$analysis$s.names, SMD_S_PPI_AdminRoute$analysis$s.nlevels[[which(SMD_S_PPI_AdminRoute$analysis$s.names == 'Strain/StudyId')]], 0)` | `r ifelse('Strain/StudyId' %in% SMD_S_PPI_AdminRoute$analysis$s.names, SMD_S_PPI_AdminRoute$analysis$sigma2[[which(SMD_S_PPI_AdminRoute$analysis$s.names == 'Strain/StudyId')]], NA)` |
| Study x Strain x Experiment | `r ifelse('ExperimentID_I' %in% SMD_S_PPI_AdminRoute$analysis$s.names, SMD_S_PPI_AdminRoute$analysis$s.nlevels[[which(SMD_S_PPI_AdminRoute$analysis$s.names == 'ExperimentID_I')]], 0)` | `r ifelse('ExperimentID_I' %in% SMD_S_PPI_AdminRoute$analysis$s.names, SMD_S_PPI_AdminRoute$analysis$sigma2[[which(SMD_S_PPI_AdminRoute$analysis$s.names == 'ExperimentID_I')]], 'NA')` |
...
```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide', out.width = "100%"}
SMD_S_PPI_Drug <- subgroup_analysis_con(df, "TvC", "Prepulse inhibition", "DrugName", 0.5)
```

```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
forest_subgroup(SMD_S_PPI_Drug$plotdata, "DrugName", "Prepulse inhibition","Drug")
```

```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
SMD_S_PPI_Drug_noI <- subgroup_SMD_con(df, "TvC", "Prepulse inhibition", "DrugName", 0.5)
```

The p-value for the association between the intervention and outcome reported was `r if(round(SMD_S_PPI_Drug_noI$QMp,3)>0.001){paste0('= ',round(SMD_S_PPI_Drug_noI$QMp,3))}else{'< 0.001'}`.

| Level | Number of categories for that level included in this analysis | Attributable variance |
|:----------------------:|:----------------------:|:----------------------:|
| Strain | `r ifelse('Strain' %in% SMD_S_PPI_Drug$analysis$s.names, SMD_S_PPI_Drug$analysis$s.nlevels[[which(SMD_S_PPI_Drug$analysis$s.names == 'Strain')]], 0)` | `r ifelse('Strain' %in% SMD_S_PPI_Drug$analysis$s.names, SMD_S_PPI_Drug$analysis$sigma2[[which(SMD_S_PPI_Drug$analysis$s.names == 'Strain')]], NA)` |
| Study x Strain | `r ifelse('Strain/StudyId' %in% SMD_S_PPI_Drug$analysis$s.names, SMD_S_PPI_Drug$analysis$s.nlevels[[which(SMD_S_PPI_Drug$analysis$s.names == 'Strain/StudyId')]], 0)` | `r ifelse('Strain/StudyId' %in% SMD_S_PPI_Drug$analysis$s.names, SMD_S_PPI_Drug$analysis$sigma2[[which(SMD_S_PPI_Drug$analysis$s.names == 'Strain/StudyId')]], NA)` |
| Study x Strain x Experiment | `r ifelse('ExperimentID_I' %in% SMD_S_PPI_Drug$analysis$s.names, SMD_S_PPI_Drug$analysis$s.nlevels[[which(SMD_S_PPI_Drug$analysis$s.names == 'ExperimentID_I')]], 0)` | `r ifelse('ExperimentID_I' %in% SMD_S_PPI_Drug$analysis$s.names, SMD_S_PPI_Drug$analysis$sigma2[[which(SMD_S_PPI_Drug$analysis$s.names == 'ExperimentID_I')]], 'NA')` |
...
```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide', out.width = "100%"}
SMD_S_PPI_DrugSelectivity <- subgroup_analysis(df, "TvC", "Prepulse inhibition", "Selectivity", 0.5)
```

```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
forest_subgroup(SMD_S_PPI_DrugSelectivity$plotdata, "Selectivity", "Prepulse inhibition","High or\nLow Selectivity")
```

```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
SMD_S_PPI_DrugSelectivity_noI <- subgroup_SMD(df, "TvC", "Prepulse inhibition", "Selectivity", 0.5)
```

The p-value for the association between whether the drug was highly selective, or also manifests 5-HT1A effects, was `r if(round(SMD_S_PPI_DrugSelectivity_noI$QMp,3)>0.001){paste0('= ',round(SMD_S_PPI_DrugSelectivity_noI$QMp,3))}else{'< 0.001'}`.

| Level | Number of categories for that level included in this analysis | Attributable variance |
|:----------------------:|:----------------------:|:----------------------:|
| Strain | `r ifelse('Strain' %in% SMD_S_PPI_DrugSelectivity$analysis$s.names, SMD_S_PPI_DrugSelectivity$analysis$s.nlevels[[which(SMD_S_PPI_DrugSelectivity$analysis$s.names == 'Strain')]], 0)` | `r ifelse('Strain' %in% SMD_S_PPI_DrugSelectivity$analysis$s.names, SMD_S_PPI_DrugSelectivity$analysis$sigma2[[which(SMD_S_PPI_DrugSelectivity$analysis$s.names == 'Strain')]], NA)` |
| Study x Strain | `r ifelse('Strain/StudyId' %in% SMD_S_PPI_DrugSelectivity$analysis$s.names, SMD_S_PPI_DrugSelectivity$analysis$s.nlevels[[which(SMD_S_PPI_DrugSelectivity$analysis$s.names == 'Strain/StudyId')]], 0)` | `r ifelse('Strain/StudyId' %in% SMD_S_PPI_DrugSelectivity$analysis$s.names, SMD_S_PPI_DrugSelectivity$analysis$sigma2[[which(SMD_S_PPI_DrugSelectivity$analysis$s.names == 'Strain/StudyId')]], NA)` |
| Study x Strain x Experiment | `r ifelse('ExperimentID_I' %in% SMD_S_PPI_DrugSelectivity$analysis$s.names, SMD_S_PPI_DrugSelectivity$analysis$s.nlevels[[which(SMD_S_PPI_DrugSelectivity$analysis$s.names == 'ExperimentID_I')]], 0)` | `r ifelse('ExperimentID_I' %in% SMD_S_PPI_DrugSelectivity$analysis$s.names, SMD_S_PPI_DrugSelectivity$analysis$sigma2[[which(SMD_S_PPI_DrugSelectivity$analysis$s.names == 'ExperimentID_I')]], 'NA')` |
...
SMD_S_PPI_potency <- metaregression_analysis(df, "TvC", "Prepulse inhibition", "pE50", 0.5)
...
```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE,}
SMD_S_PPI_potency$regression_plot
```

The estimate for $\beta$ was `r SMD_S_PPI_potency$metaregression_summary$beta[2]` (p = `r  if(round(SMD_S_PPI_potency$metaregression_summary$pval[2],3)>0.001){paste0('= ',round(SMD_S_PPI_potency$metaregression_summary$pval[2],3))}else{'< 0.001'}`).

| Level | Number of categories for that level included in this analysis | Attributable variance |
|:----------------------:|:----------------------:|:----------------------:|
| Strain | `r ifelse('Strain' %in% SMD_S_PPI_potency$metaregression_summary$s.names, SMD_S_PPI_potency$metaregression_summary$s.nlevels[[which(SMD_S_PPI_potency$metaregression_summary$s.names == 'Strain')]], 0)` | `r ifelse('Strain' %in% SMD_S_PPI_potency$metaregression_summary$s.names, SMD_S_PPI_potency$metaregression_summary$sigma2[[which(SMD_S_PPI_potency$metaregression_summary$s.names == 'Strain')]], NA)` |
| Study x Strain | `r ifelse('Strain/StudyId' %in% SMD_S_PPI_potency$metaregression_summary$s.names, SMD_S_PPI_potency$metaregression_summary$s.nlevels[[which(SMD_S_PPI_potency$metaregression_summary$s.names == 'Strain/StudyId')]], 0)` | `r ifelse('Strain/StudyId' %in% SMD_S_PPI_potency$metaregression_summary$s.names, SMD_S_PPI_potency$metaregression_summary$sigma2[[which(SMD_S_PPI_potency$metaregression_summary$s.names == 'Strain/StudyId')]], NA)` |
| Study x Strain x Experiment | `r ifelse('ExperimentID_I' %in% SMD_S_PPI_potency$metaregression_summary$s.names, SMD_S_PPI_potency$metaregression_summary$s.nlevels[[which(SMD_S_PPI_potency$metaregression_summary$s.names == 'ExperimentID_I')]], 0)` | `r ifelse('ExperimentID_I' %in% SMD_S_PPI_potency$metaregression_summary$s.names, SMD_S_PPI_potency$metaregression_summary$sigma2[[which(SMD_S_PPI_potency$metaregression_summary$s.names == 'ExperimentID_I')]], 'NA')` |

#### Dose of intervention

In this iteration of the review, the TAAR1 agonists tested against control for their effect on Pre pulse inhibition were; **`r drugs <- df %>% filter(SortLabel == "TvC") %>% filter(outcome_type == "Prepulse inhibition") %>% group_by(DrugName) %>% summarise(count = n()) %>% arrange(desc(count)) %>% pull(DrugName); if (length(drugs) > 1) {paste(paste(head(drugs, -1), collapse = ", "), "and", tail(drugs, 1))} else {drugs}`**. Meta-analysis was conducted where data were available from more than nine experiments in more than two publications, and in this iteration of the review, only SEP-363856 met that threshold.

```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
SMD_S_PPI_SEP363856_dose <- metaregression_analysis_by_drug(df, "TvC",  "Prepulse inhibition", "SEP-363856", "DoseOfIntervention_mgkg", 0.5)
```
...
```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
   metaregression_plot_by_drug(SMD_S_PPI_SEP363856_dose, df, "TvC",  "Prepulse inhibition","DoseOfIntervention_mgkg", "SEP-363856")
```

The estimate for $\beta$ was `r SMD_S_PPI_SEP363856_dose$beta[2]` (p `r  if(round(SMD_S_PPI_SEP363856_dose$pval[2],3) > 0.001){paste0('= ',round(SMD_S_PPI_SEP363856_dose$pval[2],3))}else{'< 0.001'}`).
...
```{r label = 'ZH8651 ppi regression', message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
SMD_S_LMA_ZH8651_dose <- metaregression_analysis_by_drug(df, "TvC",  "Prepulse inhibition", "ZH8651", "DoseOfIntervention_mgkg", 0.5)
```
...
```{r label = 'ZH8659 ppi regression', message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
SMD_S_LMA_ZH8659_dose <- metaregression_analysis_by_drug(df, "TvC",  "Prepulse inhibition", "ZH8659", "DoseOfIntervention_mgkg", 0.5)
```
...
SMD_S_PPI_StandardDose <- metaregression_analysis(df, "TvC", "Prepulse inhibition", "StandardisedDose", 0.5)
...
```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE,}
SMD_S_PPI_StandardDose$regression_plot
```

The estimate for the change in effect per log unit change in standardised dose was `r SMD_S_PPI_StandardDose$metaregression_summary$beta[2]` (p `r if(round(SMD_S_PPI_StandardDose[["metaregression"]][["pval"]][2],3)>0.001){paste0('= ',round(SMD_S_PPI_StandardDose[["metaregression"]][["pval"]][2],3))}else{'< 0.001'}`).

| Level | Number of categories for that level included in this analysis | Attributable variance |
|:----------------------:|:----------------------:|:----------------------:|
| Strain | `r ifelse('Strain' %in% SMD_S_PPI_StandardDose$metaregression$s.names, SMD_S_PPI_StandardDose$metaregression$s.nlevels[[which(SMD_S_PPI_StandardDose$metaregression$s.names == 'Strain')]], 0)` | `r ifelse('Strain' %in% SMD_S_PPI_StandardDose$metaregression$s.names, SMD_S_PPI_StandardDose$metaregression$sigma2[[which(SMD_S_PPI_StandardDose$metaregression$s.names == 'Strain')]], NA)` |
| Study x Strain | `r ifelse('Strain/StudyId' %in% SMD_S_PPI_StandardDose$metaregression$s.names, SMD_S_PPI_StandardDose$metaregression$s.nlevels[[which(SMD_S_PPI_StandardDose$metaregression$s.names == 'Strain/StudyId')]], 0)` | `r ifelse('Strain/StudyId' %in% SMD_S_PPI_StandardDose$metaregression$s.names, SMD_S_PPI_StandardDose$metaregression$sigma2[[which(SMD_S_PPI_StandardDose$metaregression$s.names == 'Strain/StudyId')]], NA)` |
| Study x Strain x Experiment | `r ifelse('ExperimentID_I' %in% SMD_S_PPI_StandardDose$metaregression$s.names, SMD_S_PPI_StandardDose$metaregression$s.nlevels[[which(SMD_S_PPI_StandardDose$metaregression$s.names == 'ExperimentID_I')]], 0)` | `r ifelse('ExperimentID_I' %in% SMD_S_PPI_StandardDose$metaregression$s.names, SMD_S_PPI_StandardDose$metaregression$sigma2[[which(SMD_S_PPI_StandardDose$metaregression$s.names == 'ExperimentID_I')]], 'NA')` |
...
```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide', out.width = "100%"}
SMD_S_PPI_SyRCLERoB <- subgroup_analysis(df, "TvC", "Prepulse inhibition", "RoBScore", 0.5)
```

```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
forest_subgroup(SMD_S_PPI_SyRCLERoB$plotdata, "RoBScore", "Prepulse inhibition","SyRCLE RoB score")
```

```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
SMD_S_PPI_SyRCLERoB_noI <- subgroup_SMD(df, "TvC", "Prepulse inhibition", "RoBScore", 0.5)
```

The p-value for the association between SyRCLE Risks of Bias reporting and outcome reported was `r if(round(SMD_S_PPI_SyRCLERoB_noI$QMp,3)>0.001){paste0('= ',round(SMD_S_PPI_SyRCLERoB_noI$QMp,3))}else{'< 0.001'}`.

| Level | Number of categories for that level included in this analysis | Attributable variance |
|:----------------------:|:----------------------:|:----------------------:|
| Strain | `r ifelse('Strain' %in% SMD_S_PPI_SyRCLERoB$analysis$s.names, SMD_S_PPI_SyRCLERoB$analysis$s.nlevels[[which(SMD_S_PPI_SyRCLERoB$analysis$s.names == 'Strain')]], 0)` | `r ifelse('Strain' %in% SMD_S_PPI_SyRCLERoB$analysis$s.names, SMD_S_PPI_SyRCLERoB$analysis$sigma2[[which(SMD_S_PPI_SyRCLERoB$analysis$s.names == 'Strain')]], NA)` |
| Study x Strain | `r ifelse('Strain/StudyId' %in% SMD_S_PPI_SyRCLERoB$analysis$s.names, SMD_S_PPI_SyRCLERoB$analysis$s.nlevels[[which(SMD_S_PPI_SyRCLERoB$analysis$s.names == 'Strain/StudyId')]], 0)` | `r ifelse('Strain/StudyId' %in% SMD_S_PPI_SyRCLERoB$analysis$s.names, SMD_S_PPI_SyRCLERoB$analysis$sigma2[[which(SMD_S_PPI_SyRCLERoB$analysis$s.names == 'Strain/StudyId')]], NA)` |
| Study x Strain x Experiment | `r ifelse('ExperimentID_I' %in% SMD_S_PPI_SyRCLERoB$analysis$s.names, SMD_S_PPI_SyRCLERoB$analysis$s.nlevels[[which(SMD_S_PPI_SyRCLERoB$analysis$s.names == 'ExperimentID_I')]], 0)` | `r ifelse('ExperimentID_I' %in% SMD_S_PPI_SyRCLERoB$analysis$s.names, SMD_S_PPI_SyRCLERoB$analysis$sigma2[[which(SMD_S_PPI_SyRCLERoB$analysis$s.names == 'ExperimentID_I')]], 'NA')` |
...
```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide', out.width = "100%"}
SMD_S_PPI_SyRCLERoBTF <- subgroup_analysis(df, "TvC", "Prepulse inhibition", "RoBTF", 0.5)
```

```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
forest_subgroup(SMD_S_PPI_SyRCLERoBTF$plotdata, "RoBTF", "Prepulse inhibition","RoB score")
```

```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
SMD_S_PPI_SyRCLERoBTF_noI <- subgroup_SMD(df, "TvC", "Prepulse inhibition", "RoBTF", 0.5)
```

The p-value for the association between low SyRCLE Risks of Bias reporting and outcome reported was `r if(round(SMD_S_PPI_SyRCLERoBTF_noI$QMp,3)>0.001){paste0('= ',round(SMD_S_PPI_SyRCLERoBTF_noI$QMp,3))}else{'< 0.001'}`.

| Level | Number of categories for that level included in this analysis | Attributable variance |
|:----------------------:|:----------------------:|:----------------------:|
| Strain | `r ifelse('Strain' %in% SMD_S_PPI_SyRCLERoBTF$analysis$s.names, SMD_S_PPI_SyRCLERoBTF$analysis$s.nlevels[[which(SMD_S_PPI_SyRCLERoBTF$analysis$s.names == 'Strain')]], 0)` | `r ifelse('Strain' %in% SMD_S_PPI_SyRCLERoBTF$analysis$s.names, SMD_S_PPI_SyRCLERoBTF$analysis$sigma2[[which(SMD_S_PPI_SyRCLERoBTF$analysis$s.names == 'Strain')]], NA)` |
| Study x Strain | `r ifelse('Strain/StudyId' %in% SMD_S_PPI_SyRCLERoBTF$analysis$s.names, SMD_S_PPI_SyRCLERoBTF$analysis$s.nlevels[[which(SMD_S_PPI_SyRCLERoBTF$analysis$s.names == 'Strain/StudyId')]], 0)` | `r ifelse('Strain/StudyId' %in% SMD_S_PPI_SyRCLERoBTF$analysis$s.names, SMD_S_PPI_SyRCLERoBTF$analysis$sigma2[[which(SMD_S_PPI_SyRCLERoBTF$analysis$s.names == 'Strain/StudyId')]], NA)` |
| Study x Strain x Experiment | `r ifelse('ExperimentID_I' %in% SMD_S_PPI_SyRCLERoBTF$analysis$s.names, SMD_S_PPI_SyRCLERoBTF$analysis$s.nlevels[[which(SMD_S_PPI_SyRCLERoBTF$analysis$s.names == 'ExperimentID_I')]], 0)` | `r ifelse('ExperimentID_I' %in% SMD_S_PPI_SyRCLERoBTF$analysis$s.names, SMD_S_PPI_SyRCLERoBTF$analysis$sigma2[[which(SMD_S_PPI_SyRCLERoBTF$analysis$s.names == 'ExperimentID_I')]], 'NA')` |

#### ARRIVE reporting completeness guidelines

Figure 2.2.4.10 displays a visualisation of the meta-regression using the number of ARRIVE items met (from a possible total of 22) as an explanatory variable. Dashed lines represent the 95% confidence interval of the regression line. The dotted lines represent the 95% prediction interval. Raw data are plotted with 'bubble' size adjusted according to effect size precision.

```{r label = 'TvC, PPI, ARRIVE metaregression', message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
SMD_S_PPI_ARR2 <- metaregression_analysis_con(df, "TvC", "Prepulse inhibition", "ARRIVEScore", 0.5)
...
```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE,}
SMD_S_PPI_ARR2$regression_plot
```

The estimate for $\beta$ was `r SMD_S_PPI_ARR2$metaregression_summary$beta[2]` (p`r if(round(SMD_S_PPI_ARR2$metaregression_summary$pval[2],3)>0.001){paste0('= ',round(SMD_S_PPI_ARR2$metaregression_summary$pval[2],3))}else{'< 0.001'}`).

| Level | Number of categories for that level included in this analysis | Attributable variance |
|:----------------------:|:----------------------:|:----------------------:|
| Strain | `r ifelse('Strain' %in% SMD_S_PPI_ARR2$metaregression$s.names, SMD_S_PPI_ARR2$metaregression$s.nlevels[[which(SMD_S_PPI_ARR2$metaregression$s.names == 'Strain')]], 0)` | `r ifelse('Strain' %in% SMD_S_PPI_ARR2$metaregression$s.names, SMD_S_PPI_ARR2$metaregression$sigma2[[which(SMD_S_PPI_ARR2$metaregression$s.names == 'Strain')]], NA)` |
| Study x Strain | `r ifelse('Strain/StudyId' %in% SMD_S_PPI_ARR2$metaregression$s.names, SMD_S_PPI_ARR2$metaregression$s.nlevels[[which(SMD_S_PPI_ARR2$metaregression$s.names == 'Strain/StudyId')]], 0)` | `r ifelse('Strain/StudyId' %in% SMD_S_PPI_ARR2$metaregression$s.names, SMD_S_PPI_ARR2$metaregression$sigma2[[which(SMD_S_PPI_ARR2$metaregression$s.names == 'Strain/StudyId')]], NA)` |
| Study x Strain x Experiment | `r ifelse('ExperimentID_I' %in% SMD_S_PPI_ARR2$metaregression$s.names, SMD_S_PPI_ARR2$metaregression$s.nlevels[[which(SMD_S_PPI_ARR2$metaregression$s.names == 'ExperimentID_I')]], 0)` | `r ifelse('ExperimentID_I' %in% SMD_S_PPI_ARR2$metaregression$s.names, SMD_S_PPI_ARR2$metaregression$sigma2[[which(SMD_S_PPI_ARR2$metaregression$s.names == 'ExperimentID_I')]], 'NA')` |

#### Heterogeneity explained by covariates (TAAR1 Agonist vs Control on Pre pulse inhibition)

```{r label = 'TvC PPI Het', message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide', out.width = "100%"}
SMD_S_PPI_sexI <- subgroup_SMDI(df, "TvC", "Prepulse inhibition", "Sex", 0.5)
SMD_S_PPI_AdminRouteI <- subgroup_SMDI(df, "TvC", "Prepulse inhibition", "InterventionAdministrationRoute", 0.5)
SMD_S_PPI_ProphTheraI <- subgroup_SMDI(df, "TvC", "Prepulse inhibition", "ProphylacticOrTherapeutic", 0.5)
SMD_S_PPI_DrugI <- subgroup_SMDI_con(df, "TvC", "Prepulse inhibition", "DrugName", 0.5)
SMD_S_PPI_SyRCLERoBI <- subgroup_SMDI(df, "TvC", "Prepulse inhibition", "RoBScore", 0.5)
SMD_S_PPI_DrugSelectivityI <- subgroup_SMDI(df, "TvC", "Prepulse inhibition", "Selectivity", 0.5)
```
...
| Moderator | Category | $\beta$ | 95% CI | Marginal R^2^ (%) |
|:-------------:|:-------------:|:-------------:|:-------------:|:-------------:|
| Overall effect | \- | `r SMD_S_PPI$beta[1]` | `r SMD_S_PPI$ci.lb` to `r SMD_S_PPI$ci.ub` | \- |
| Sex | \- | \- | \- | `r round((r2_ml(SMD_S_PPI_sexI)[1]*100),1)`% |
| \- | *Female* | `r SMD_S_PPI_sexI$beta[1]` | `r SMD_S_PPI_sexI$ci.lb[1]` to `r SMD_S_PPI_sexI$ci.ub[1]` | \- |
| \- | *Male* | `r SMD_S_PPI_sexI$beta[2]` | `r SMD_S_PPI_sexI$ci.lb[2]` to `r SMD_S_PPI_sexI$ci.ub[2]` | \- |
| \- | *Mixed male and female* | `r SMD_S_PPI_sexI$beta[3]` | `r SMD_S_PPI_sexI$ci.lb[3]` to `r SMD_S_PPI_sexI$ci.ub[3]` | \- |
| Administration route | \- | \- | \- | `r round((r2_ml(SMD_S_PPI_AdminRouteI)[1]*100),1)`% |
| \- | *Intraperitoneal* | `r SMD_S_PPI_AdminRouteI$beta[1]` | `r SMD_S_PPI_AdminRouteI$ci.lb[1]` to `r SMD_S_PPI_AdminRouteI$ci.ub[1]` | \- |
| \- | *Oral* | `r SMD_S_PPI_AdminRouteI$beta[2]` | `r SMD_S_PPI_AdminRouteI$ci.lb[2]` to `r SMD_S_PPI_AdminRouteI$ci.ub[2]` | \- |
| \- | *Subcutaneous* | `r SMD_S_PPI_AdminRouteI$beta[3]` | `r SMD_S_PPI_AdminRouteI$ci.lb[3]` to `r SMD_S_PPI_AdminRouteI$ci.ub[3]` | \- |
| Intervention administered | \- | \- | \- | `r round((r2_ml(SMD_S_PPI_DrugI)[1]*100),1)`% |
| \- | *SEP-363856 (Ultaront)* | `r SMD_S_PPI_DrugI$beta[1]` | `r SMD_S_PPI_DrugI$ci.lb[1]` to `r SMD_S_PPI_DrugI$ci.ub[1]` | \- |
| \- | *ZH8651* | `r SMD_S_PPI_DrugI$beta[2]` | `r SMD_S_PPI_DrugI$ci.lb[2]` to `r SMD_S_PPI_DrugI$ci.ub[2]` | \- |
| \- | *ZH8659* | `r SMD_S_PPI_DrugI$beta[3]` | `r SMD_S_PPI_DrugI$ci.lb[3]` to `r SMD_S_PPI_DrugI$ci.ub[3]` | \- |
| Drug selectivity | \- | \- | \- | `r round((r2_ml(SMD_S_PPI_DrugSelectivityI)[1]*100),1)`% |
| \- | *High* | `r SMD_S_PPI_DrugSelectivityI$beta[1]` | `r SMD_S_PPI_DrugSelectivityI$ci.lb[1]` to `r SMD_S_PPI_DrugSelectivityI$ci.ub[1]` | \- |
| \- | *Low* | `r SMD_S_PPI_DrugSelectivityI$beta[2]` | `r SMD_S_PPI_DrugSelectivityI$ci.lb[2]` to `r SMD_S_PPI_DrugSelectivityI$ci.ub[2]` | \- |
| Drug potency | per log unit | `r SMD_S_PPI_potency$metaregression$beta[2]` | `r SMD_S_PPI_potency$metaregression$ci.lb[2]` to `r SMD_S_PPI_potency$metaregression$ci.ub[2]` | `r round((r2_ml(SMD_S_PPI_potency$metaregression)[1]*100),1)`% |
| Standardised dose | per log unit | `r SMD_S_PPI_StandardDose$metaregression$beta[2]` | `r SMD_S_PPI_StandardDose$metaregression$ci.lb[2]` to `r SMD_S_PPI_StandardDose$metaregression$ci.ub[2]` | `r round((r2_ml(SMD_S_PPI_StandardDose$metaregression)[1]*100),1)`% |
| Risk of Bias | \- | \- | \- | `r round((r2_ml(SMD_S_PPI_SyRCLERoBI)[1]*100),1)`% |
| \- | *0 criteria met* | `r SMD_S_PPI_SyRCLERoBI$beta[1]` | `r SMD_S_PPI_SyRCLERoBI$ci.lb[1]` to `r SMD_S_PPI_SyRCLERoBI$ci.ub[1]` | \- |
| \- | *1 criteria met* | `r SMD_S_PPI_SyRCLERoBI$beta[2]` | `r SMD_S_PPI_SyRCLERoBI$ci.lb[2]` to `r SMD_S_PPI_SyRCLERoBI$ci.ub[2]` | \- |
| Reporting completeness | per log unit | `r SMD_S_PPI_ARR2$metaregression$beta[2]` | `r SMD_S_PPI_ARR2$metaregression$ci.lb[2]` to `r SMD_S_PPI_ARR2$metaregression$ci.ub[2]` | `r round((r2_ml(SMD_S_PPI_ARR2$metaregression)[1]*100),1)`% |
...
```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
SMD_S_PPI_rho0.2 <- run_ML_SMD(df, "TvC", "Prepulse inhibition", 0.2)
```

When the $\rho$ value is assumed to be 0.2, the TAAR1 interventions had a pooled effect on Pre pulse inhibition of **SMD = `r SMD_S_PPI_rho0.2[["beta"]]`** (95% CI: `r SMD_S_PPI_rho0.2[["ci.lb"]]` to `r SMD_S_PPI_rho0.2[["ci.ub"]]`) with a prediction interval of `r predict(SMD_S_PPI_rho0.2)$pi.lb` to `r predict(SMD_S_PPI_rho0.2)$pi.ub`).

```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
SMD_S_PPI_rho0.8 <- run_ML_SMD(df, "TvC", "Prepulse inhibition", 0.8)
```

When the $\rho$ value is assumed to be 0.8, the TAAR1 interventions had a pooled effect on Pre pulse inhibition of **SMD = `r SMD_S_PPI_rho0.8[["beta"]]`** (95% CI: `r SMD_S_PPI_rho0.8[["ci.lb"]]` to `r SMD_S_PPI_rho0.8[["ci.ub"]]`) with a prediction interval of `r predict(SMD_S_PPI_rho0.8)$pi.lb` to `r predict(SMD_S_PPI_rho0.8)$pi.ub`).

For reference the pooled effect size when rho is assumed to be 0.5 is `r SMD_S_PPI[["beta"]]` (95% CI: `r SMD_S_PPI[["ci.lb"]]` to `r SMD_S_PPI[["ci.ub"]]`).

#### Normalised Mean Difference (NMD)

For Pre pulse inhibition, `r df_S_PPI %>% filter(NMD_possible == "TRUE") %>% nrow()` out of `r nrow(df_S_PPI)` comparisons, i.e. `r round(((df_S_PPI %>% filter(NMD_possible == "TRUE") %>% nrow())/(nrow(df_S_PPI)))*100,1)` % of comparisons, had data available for a Sham group, and for these studies it was possible to calculate an NMD estimate of effect size.
...
```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
NMD_S_PPI <- run_ML_NMD(df, "TvC", "Prepulse inhibition", 0.5)
```

**Figure 2.2.5 - Forest plot of TAAR1 agonist vs control on Pre pulse inhibition using NMD**

```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, fig.height=18, fig.width=16}
forest_metafor_NMD(NMD_S_PPI, "Prepulse inhibition")
```

For TAAR1 Agonist v Control, TAAR1 interventions had a pooled effect on Pre pulse inhibition of NMD = `r round(NMD_S_PPI[["beta"]][1],1)` (95% CI: `r round(NMD_S_PPI[["ci.lb"]],1)` to `r round(NMD_S_PPI[["ci.ub"]],1)`) with a prediction interval of `r round(predict(NMD_S_PPI)$pi.lb,1)` to `r round(predict(NMD_S_PPI)$pi.ub,1)`. For reference the pooled effect size for SMD was `r SMD_S_PPI[["beta"]]` (95% CI: `r SMD_S_PPI[["ci.lb"]]` to `r SMD_S_PPI[["ci.ub"]]`).

`r NMD_S_PPI[["k"]]` experimental comparisons were reported in `r length(unique(NMD_S_PPI$data$ExperimentID_I))` experiments reported from `r length(unique(NMD_S_PPI$data$StudyId))` publications and involving `r length(unique(NMD_S_PPI$data$Strain))` different animal strains.

| Level | Number of categories for that level included in this analysis | Attributable variance |
|:----------------------:|:----------------------:|:----------------------:|
| Strain | `r ifelse('Strain' %in% NMD_S_PPI$s.names, NMD_S_PPI$s.nlevels[[which(NMD_S_PPI$s.names == 'Strain')]], 0)` | `r ifelse('Strain' %in% NMD_S_PPI$s.names, NMD_S_PPI$sigma2[[which(NMD_S_PPI$s.names == 'Strain')]], NA)` |
| Study x Strain | `r ifelse('Strain/StudyId' %in% NMD_S_PPI$s.names, NMD_S_PPI$s.nlevels[[which(NMD_S_PPI$s.names == 'Strain/StudyId')]], 0)` | `r ifelse('Strain/StudyId' %in% NMD_S_PPI$s.names, NMD_S_PPI$sigma2[[which(NMD_S_PPI$s.names == 'Strain/StudyId')]], NA)` |
| Study x Strain x Experiment | `r ifelse('ExperimentID_I' %in% NMD_S_PPI$s.names, NMD_S_PPI$s.nlevels[[which(NMD_S_PPI$s.names == 'ExperimentID_I')]], 0)` | `r ifelse('ExperimentID_I' %in% NMD_S_PPI$s.names, NMD_S_PPI$sigma2[[which(NMD_S_PPI$s.names == 'ExperimentID_I')]], 'NA')` |
...
```{r warning=FALSE, eval = TRUE, echo = FALSE}
SMD_S_PPI_RVE <- robust(SMD_S_PPI, cluster = Strain, clubSandwich=T, verbose=T)
```

When using the robust variance estimator, TAAR1 interventions had a pooled effect on Pre pulse inhibition of SMD = `r SMD_S_PPI_RVE[["beta"]][1]` (95% CI: `r SMD_S_PPI_RVE[["ci.lb"]]` to `r SMD_S_PPI_RVE[["ci.ub"]]` with a prediction interval of `r predict(SMD_S_PPI_RVE)$pi.lb` to `r predict(SMD_S_PPI_RVE)$pi.ub`). For reference the pooled effect size for SMD was `r SMD_S_PPI[["beta"]][1]` (95% CI: `r SMD_S_PPI[["ci.lb"]]` to `r SMD_S_PPI[["ci.ub"]]`), so the using a robust variance estimator does not substantially change the results, although the confidence intervals are wider.
...
```{r warning=FALSE, eval = TRUE, message = FALSE, echo = FALSE}
run_sse_plot_SMD_P(df)
#run_sse_NMD(df)
```

Egger regression based on `r run_sse_SMD_P(df)[["k"]]` effects of TAAR1 Agonist v Control where Pre pulse inhibition was measured showed a coefficient for a small study effect of `r run_sse_SMD_P(df)[["beta"]][2]` (95% CI: `r run_sse_SMD_P(df)[["ci.lb"]][2]` to `r run_sse_SMD_P(df)[["ci.ub"]][2]`; p = `r ifelse(run_sse_SMD_P(df)[["pval"]][2] < 0.001, "<0.001", sprintf("%.3f", run_sse_SMD_P(df)[["pval"]][2]))`).

| Level | Number of categories for that level included in this analysis | Attributable variance |
|:----------------------:|:----------------------:|:----------------------:|
| Strain | `r ifelse('Strain' %in% run_sse_SMD_P(df)$s.names, run_sse_SMD_P(df)$s.nlevels[[which(run_sse_SMD_P(df)$s.names == 'Strain')]], 0)` | `r ifelse('Strain' %in% run_sse_SMD_P(df)$s.names, run_sse_SMD_P(df)$sigma2[[which(run_sse_SMD_P(df)$s.names == 'Strain')]], NA)` |
| Study x Strain | `r ifelse('Strain/StudyId' %in% run_sse_SMD_P(df)$s.names, run_sse_SMD_P(df)$s.nlevels[[which(run_sse_SMD_P(df)$s.names == 'Strain/StudyId')]], 0)` | `r ifelse('Strain/StudyId' %in% run_sse_SMD_P(df)$s.names, run_sse_SMD_P(df)$sigma2[[which(run_sse_SMD_P(df)$s.names == 'Strain/StudyId')]], NA)` |
| Study x Strain x Experiment | `r ifelse('ExperimentID_I' %in% run_sse_SMD_P(df)$s.names, run_sse_SMD_P(df)$s.nlevels[[which(run_sse_SMD_P(df)$s.names == 'ExperimentID_I')]], 0)` | `r ifelse('ExperimentID_I' %in% run_sse_SMD_P(df)$s.names, run_sse_SMD_P(df)$sigma2[[which(run_sse_SMD_P(df)$s.names == 'ExperimentID_I')]], 'NA')` |
...
if (any(class(SMD_S_Cog) == "rma.mv")) {
  output_text1 <- "Figure 2.3.3 - Forest plot of cognitive function for TAAR1 Agonist vs control"
} else {
  output_text1 <- ""
}
```
...
```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide', out.width = "100%"}
SMD_S_Cog_Drug <- subgroup_analysis_con(df, "TvC", "Cognition", "DrugName", 0.5)
```
...
```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
SMD_S_Cog_Drug_noI <- subgroup_SMD_con(df, "TvC", "Cognition", "DrugName", 0.5)
```
...
The estimate for $\beta$ was `r SMD_S_Cog_SEP363856_dose$beta[2]` (p = `r if(round(SMD_S_Cog_SEP363856_dose$pval[2],3)>0.001){paste0('= ',round(SMD_S_Cog_SEP363856_dose$pval[2],3))}else{'< 0.001'}`).

| Level | Number of categories for that level included in this analysis | Attributable variance |
|:----------------------:|:----------------------:|:----------------------:|
| Strain | `r ifelse('Strain' %in% SMD_S_Cog_SEP363856_dose$s.names, SMD_S_Cog_SEP363856_dose$s.nlevels[[which(SMD_S_Cog_SEP363856_dose$s.names == 'Strain')]], 0)` | `r ifelse('Strain' %in% SMD_S_Cog_SEP363856_dose$s.names, SMD_S_Cog_SEP363856_dose$sigma2[[which(SMD_S_Cog_SEP363856_dose$s.names == 'Strain')]], NA)` |
| Study x Strain | `r ifelse('Strain/StudyId' %in% SMD_S_Cog_SEP363856_dose$s.names, SMD_S_Cog_SEP363856_dose$s.nlevels[[which(SMD_S_Cog_SEP363856_dose$s.names == 'Strain/StudyId')]], 0)` | `r ifelse('Strain/StudyId' %in% SMD_S_Cog_SEP363856_dose$s.names, SMD_S_Cog_SEP363856_dose$sigma2[[which(SMD_S_Cog_SEP363856_dose$s.names == 'Strain/StudyId')]], NA)` |
| Study x Strain x Experiment | `r ifelse('Strain/StudyId/ExperimentID_I' %in% SMD_S_Cog_SEP363856_dose$s.names, SMD_S_Cog_SEP363856_dose$s.nlevels[[which(SMD_S_Cog_SEP363856_dose$s.names == 'Strain/StudyId/ExperimentID_I')]], 0)` | `r ifelse('Strain/StudyId/ExperimentID_I' %in% SMD_S_Cog_SEP363856_dose$s.names, SMD_S_Cog_SEP363856_dose$sigma2[[which(SMD_S_Cog_SEP363856_dose$s.names == 'Strain/StudyId/ExperimentID_I')]], 'NA')` |
...
```{r label = 'SMD_S_Cog_StandardDose', message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}

SMD_S_Cog_StandardDose <- metaregression_analysis_con(df, "TvC", "Cognition", "StandardisedDose", 0.5)
...
```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
SMD_S_cog_ARR2 <- metaregression_analysis_con(df, "TvC", "Cognition", "ARRIVEScore", 0.5)
...
SMD_S_Cog_DrugI <- subgroup_SMDI_con(df, "TvC", "Cognition", "DrugName", 0.5)
...
| \- | *Compound 6e* | `r SMD_S_Cog_DrugI$beta[1]` | `r SMD_S_Cog_DrugI$ci.lb[1]` to `r SMD_S_Cog_DrugI$ci.ub[1]` | \- |
| \- | *Compound 7b* | `r SMD_S_Cog_DrugI$beta[2]` | `r SMD_S_Cog_DrugI$ci.lb[2]` to `r SMD_S_Cog_DrugI$ci.ub[2]` | \- |
| \- | *Compound 8b* | `r SMD_S_Cog_DrugI$beta[3]` | `r SMD_S_Cog_DrugI$ci.lb[3]` to `r SMD_S_Cog_DrugI$ci.ub[3]` | \- |
| \- | *RO5203648* | `r SMD_S_Cog_DrugI$beta[4]` | `r SMD_S_Cog_DrugI$ci.lb[4]` to `r SMD_S_Cog_DrugI$ci.ub[4]` | \- |
| \- | *RO5256390* | `r SMD_S_Cog_DrugI$beta[5]` | `r SMD_S_Cog_DrugI$ci.lb[5]` to `r SMD_S_Cog_DrugI$ci.ub[5]` | \- |
| \- | *SEP-363856 (Ultaront)* | `r SMD_S_Cog_DrugI$beta[6]` | `r SMD_S_Cog_DrugI$ci.lb[6]` to `r SMD_S_Cog_DrugI$ci.ub[6]` | \- |
| \- | *ZH8651* | `r SMD_S_Cog_DrugI$beta[7]` | `r SMD_S_Cog_DrugI$ci.lb[7]` to `r SMD_S_Cog_DrugI$ci.ub[7]` | \- |
| \- | *ZH8659* | `r SMD_S_Cog_DrugI$beta[8]` | `r SMD_S_Cog_DrugI$ci.lb[8]` to `r SMD_S_Cog_DrugI$ci.ub[8]` | \- |
...
For cognition, `r df_S_cog %>% filter(NMD_possible == "TRUE") %>% nrow()` out of `r nrow(df_S_cog)` comparisons, i.e. `r round(((df_S_cog %>% filter(NMD_possible == "TRUE") %>% nrow())/(nrow(df_S_cog)))*100,1)` % of comparisons, had data available for a Sham group, and for these studies it was possible to calculate an NMD estimate of effect size.
...
```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, fig.height=12, fig.width=14}
forest_metafor_NMD(NMD_S_cog, "Cognition")
```

For TAAR1 Agonist v Control, TAAR1 interventions had a pooled effect on cognition of NMD = `r round(NMD_S_cog[["beta"]][1],1)` (95% CI: `r round(NMD_S_cog[["ci.lb"]],1)` to `r round(NMD_S_cog[["ci.ub"]],1)`) with a prediction interval of `r round(predict(NMD_S_cog)$pi.lb,1)` to `r round(predict(NMD_S_cog)$pi.ub,1)`. For reference the pooled effect size for SMD was `r SMD_S_Cog[["beta"]]` (95% CI: `r SMD_S_Cog[["ci.lb"]]` to `r SMD_S_Cog[["ci.ub"]]`).
...
Egger regression based on `r run_sse_SMD_C(df)[["k"]]` effects of TAAR1 Agonist v Control where Cognitive function was measured showed a coefficient for a small study effect of `r run_sse_SMD_C(df)[["beta"]][2]` (95% CI: `r run_sse_SMD_C(df)[["ci.lb"]][2]` to `r run_sse_SMD_C(df)[["ci.ub"]][2]`; p = `r ifelse(run_sse_SMD_C(df)[["pval"]][2] < 0.001, "<0.001", sprintf("%.3f", run_sse_SMD_C(df)[["pval"]][2]))`).
...
Nsdf$`n allocated to group at experiment start?_C` <- as.numeric(Nsdf$`n allocated to group at experiment start?_C`)
Nsdf$`n allocated to group at experiment start?_I` <- as.numeric(Nsdf$`n allocated to group at experiment start?_I`)
...
```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE}
SoE_TvC <- read_xlsx("data_u1/soe_tables_20260129.xlsx", "TvC")
...
```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE}
SoE_TvA <- read_xlsx("data_u1/soe_tables_20260129.xlsx", "TvA")
...
```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE}
SoE_Indir <- read_xlsx("data_u1/soe_tables_20260129.xlsx", "Indir") ### ? where is this##
