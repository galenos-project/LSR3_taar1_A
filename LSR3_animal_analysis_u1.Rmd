...
original_data <- tab2[1:76, 1:6]


# Rows to have bold lines beneath
rows_with_bold_lines <- c(0,2,3,7,8,9,11,12,13,19,23,26,27,36,38,45,46,47,48,49,54,64,65,67,76)
...
Only two publications reported each of social interaction and stereotypy, and so these outcomes are not analysed further.
...
**Figure 2.1.4.4 - Subgroup analysis of TAAR1 agonist vs control on locomotor activity by model type**

```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
SMD_S_LMA_CDI_level2 <- subgroup_analysis(df, "TvC", "Locomotor activity", "CDI_level2", 0.5)
```

```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
forest_subgroup(SMD_S_LMA_CDI_level2$plotdata, "CDI_level2", "Locomotor Activity","Treatment before \nor after model induction")
```

```{r message=FALSE, warning=FALSE, eval = TRUE, echo = FALSE, results='hide'}
SMD_S_LMA_CDI_level2_noI <- subgroup_SMD(df, "TvC", "Locomotor activity", "CDI_level2", 0.5)
```
...
|            Level            |                                                                             Number of categories for that level included in this analysis                                                                             |                                                                                                 Attributable variance                                                                                                 |
|:----------------------:|:----------------------:|:----------------------:|
|           Strain            |                        `r ifelse('Strain' %in% SMD_S_LMA_ProphThera$analysis$s.names, SMD_S_LMA_CDI_level2$analysis$s.nlevels[[which(SMD_S_LMA_CDI_level2$analysis$s.names == 'Strain')]], 0)`                        |                         `r ifelse('Strain' %in% SMD_S_LMA_CDI_level2$analysis$s.names, SMD_S_LMA_CDI_level2$analysis$sigma2[[which(SMD_S_LMA_CDI_level2$analysis$s.names == 'Strain')]], NA)`                         |
|       Study x Strain        |                `r ifelse('Strain/StudyId' %in% SMD_S_LMA_CDI_level2$analysis$s.names, SMD_S_LMA_CDI_level2$analysis$s.nlevels[[which(SMD_S_LMA_CDI_level2$analysis$s.names == 'Strain/StudyId')]], 0)`                |                 `r ifelse('Strain/StudyId' %in% SMD_S_LMA_CDI_level2$analysis$s.names, SMD_S_LMA_CDI_level2$analysis$sigma2[[which(SMD_S_LMA_CDI_level2$analysis$s.names == 'Strain/StudyId')]], NA)`                 |
| Study x Strain x Experiment | `r ifelse('Strain/StudyId/ExperimentID_I' %in% SMD_S_LMA_CDI_level2$analysis$s.names, SMD_S_LMA_CDI_level2$analysis$s.nlevels[[which(SMD_S_LMA_CDI_level2$analysis$s.names == 'Strain/StudyId/ExperimentID_I')]], 0)` | `r ifelse('Strain/StudyId/ExperimentID_I' %in% SMD_S_LMA_CDI_level2$analysis$s.names, SMD_S_LMA_CDI_level2$analysis$sigma2[[which(SMD_S_LMA_CDI_level2$analysis$s.names == 'Strain/StudyId/ExperimentID_I')]], 'NA')` |
...
The p-value for the association between whether the drug was highly selective, or also manifests 5-HT1A or $/a$-2 adrenergic effects, and outcome reported was `r round(SMD_S_LMA_DrugSelectivity_noI$QMp,3)`.
...
|                    \-                    |    *Stimulant or Dopamine agonist*    |         `r SMD_S_LMA_PharmDisIndI$beta[2]`          |                  `r SMD_S_LMA_PharmDisIndI$ci.lb[2]` to `r SMD_S_LMA_PharmDisIndI$ci.ub[2]`                  |   |           Administration route           |           \-            |                        \-                         |                                                    \-                                                    |         `r round((r2_ml(SMD_S_LMA_AdminRouteI)[1]*100),1)`%         |
...
The estimate for $\beta$ was `r SMD_S_PPI_ARR2$metaregression_summary$beta[2]` (p if(round(SMD_S_PPI_ARR2$metaregression_summary$pval[2],3)>0.001){paste0('= ',round(SMD_S_PPI_ARR2$metaregression_summary$pval[2],3))}else{'< 0.001'}`.
...
For Pre pulse inhibition, `r df_S_PPI %>% filter(NMD_possible == "TRUE") %>% nrow()` out of `r nrow(df_S_PPI)` comparisons, i.e. `r ((df_S_PPI %>% filter(NMD_possible == "TRUE") %>% nrow())/(nrow(df_S_PPI)))*100` % of comparisons, had data available for a Sham group, and for these studies it was possible to calculate an NMD estimate of effect size.
...
For TAAR1 Agonist v Control, TAAR1 interventions had a pooled effect on Pre pulse inhibition of NMD = `r NMD_S_PPI[["beta"]][1]` (95% CI: `r NMD_S_PPI[["ci.lb"]]` to `r NMD_S_PPI[["ci.ub"]]`) with a prediction interval of `r predict(NMD_S_PPI)$pi.lb` to `r predict(NMD_S_PPI)$pi.ub`. For reference the pooled effect size for SMD was `r SMD_S_PPI[["beta"]]` (95% CI: `r SMD_S_PPI[["ci.lb"]]` to `r SMD_S_PPI[["ci.ub"]]`).
...
No studies reported pre pulse inhibition in TAAR1 knockout animals
