...
# All outcomes picked up by regex and appropriately categorised into OutcomeType
# All drug names correctly picked up, EC50 values added for new drugs
# Studies in two parts combined/paired up
# Regex correctly picking up TAAR KO, antipsychotics etc - determines comparison type
...
LSR3_SyRFOutcomes <- read_csv("data_u2/Quantitative_data_-_2025_07_16_-_c494b1ae-4cf4-4618-b91a-e69a2b815bdd_-_Investigators_Unblinded.csv")
...
LSR3_reconciled <- LSR3_reconciled %>% 
  ...
  mutate(StudyId = ifelse(StudyId == 'f16411ee-022b-4380-90a4-b9803f3d2718', '55bd4131-4668-401d-9e07-1935015bb592', StudyId))
...
#reconciled_cohort_label$IsDiseaseModelControl <- reconciled_cohort_label$IsDiseaseModelControl
#reconciled_cohort_label$ModelID <- reconciled_cohort_label$`ModelID`

# Edit for u2: remove second column when two disease models are applied
reconciled_cohort_label <- reconciled_cohort_label %>%
  select(-`IsDiseaseModelControl[2]`, -`DiseaseModelLabel(s)[2]`, -`ModelID[2]`) %>%
  rename(IsDiseaseModelControl = `IsDiseaseModelControl[1]`, 
         `DiseaseModelLabel(s)` = `DiseaseModelLabel(s)[1]`, 
         ModelID = `ModelID[1]`)
...
##### Remove DA knockouts (heterozygotes) and data reported from subgroups #####
data_rem <- subset(data, data$'DiseaseModelLabel(s)' == 'DAT +/-')
data <- anti_join(data, data_rem)
#remove data reported from subgroups
data_rem <- data %>% filter(str_detect(`DiseaseModelLabel(s)`, "SUBGROUP") | str_detect(CohortLabel, 'SUBGROUP')) # currently empty (u2)
data <- anti_join(data, data_rem)
data <- data %>%
...
data[condition, columnname13] <- "ICR (mouse)"

condition <- data$ExperimentID == '1c48a8d4-0a37-4256-aa4a-34108731a48b'
data[condition, columnname13] <- "NMRI (mouse)"

condition <- data$ExperimentID == '32c6ccc3-e4e2-4a41-98c2-f36e8984775e'
data[condition, columnname13] <- "NMRI (mouse)"

condition <- data$StudyId == 'e6051edc-1110-4f6a-bb4d-436a5cb79c71'
data[condition, columnname13] <- "Wistar (rat)"

condition <- data$StudyId == '5ec13944-5c99-413f-9070-795ea2a21e98'
data[condition, columnname13] <- "Not stated (mouse)"




#correct for some column names not brought through from SyRF
columnname14 <- 'Authors'
condition <- data$StudyId == "44702900-b89e-4c51-9ac5-b18610ab340d"
data[condition, columnname14] <- 'Leo D'

condition <- data$StudyId == "e6051edc-1110-4f6a-bb4d-436a5cb79c71"
data[condition, columnname14] <- 'Cichero E'
     
condition <- data$StudyId == "5ec13944-5c99-413f-9070-795ea2a21e98"
data[condition, columnname14] <- 'Li M'
     
condition <- data$StudyId == "30dd4627-7570-4d4d-8aaf-6db67a429496"
data[condition, columnname14] <- 'Zhou Z'
...
# Save columns that won't vary between treatment and control
data_fixed_cols <- colnames(data[,1:57]) # bibliographic and RoB/ARRIVE

# function to remove duplicate columns (when column is in fixed columns)
remove_duplicate_columns <- function(df) {
  
  # Remove columns that are in data_fixed_cols when stripped of suffix
  cols = names(df)
  x_cols = cols[grepl("\\.x$", cols)]
  y_cols = cols[grepl("\\.y$", cols)]
  x_cols_no_suffix = str_remove(x_cols, "\\.x")
  y_cols_no_suffix = str_remove(y_cols, "\\.y")
  x_cols_to_remove = x_cols[x_cols_no_suffix %in% data_fixed_cols]
  y_cols_to_remove = y_cols[y_cols_no_suffix %in% data_fixed_cols]
  df <- df %>% select(-all_of(x_cols_to_remove), -all_of(y_cols_to_remove))
  return(df)
}

data2 <- remove_duplicate_columns(data2)
...
# NOTE: REQUIRES CHECKING WITH EACH ITERATION 
dataall <- dataall %>% 
  mutate(OutcomeType = case_when(str_detect(OutcomeLabel_I, regex("lma|locomotion|loc|locomotor|oft|horizontal|distance|vertical|climbing|movement", ignore_case = TRUE)) ~ "Locomotor activity",
                                 str_detect(OutcomeLabel_I, "PPI") ~ "Prepulse inhibition",
                                 str_detect(OutcomeLabel_I, regex("morris|NOR|attention|MWM|escape|soon|reward", ignore_case = TRUE)) ~ "Cognition", 
                                 str_detect(OutcomeLabel_I, regex("stereotyp", ignore_case = TRUE)) ~ "Stereotypy",
                                 str_detect(OutcomeLabel_I, regex("social", ignore_case = TRUE)) ~ "Social interaction",
...
dataall.direction <- dataall %>% 
  ...
  rename(CategoryDiseaseInduction = `Category of disease model induction method:[1]_I`) %>%
  mutate(CategoryDiseaseInduction = case_when(CategoryDiseaseInduction == "Pharmacological (e.g. psychostimulants, NMDA antagonists)" & `Type of pharmacological induction:[1]_I` == "cocaine" ~ "Psychostimulant - Cocaine", 
                                              CategoryDiseaseInduction == "Pharmacological (e.g. psychostimulants, NMDA antagonists)" & `Type of pharmacological induction:[1]_I` == "MK-801" ~ "Psychostimulant - MK-801",
                                              CategoryDiseaseInduction == "Pharmacological (e.g. psychostimulants, NMDA antagonists)" & `Type of pharmacological induction:[1]_I` == "phencyclidine" ~ "Psychostimulant - PCP", 
                                              CategoryDiseaseInduction == "Pharmacological (e.g. psychostimulants, NMDA antagonists)" & `Type of pharmacological induction:[1]_I` == "ketamine" ~ "Psychostimulant - Ketamine", 
                                              CategoryDiseaseInduction == "Pharmacological (e.g. psychostimulants, NMDA antagonists)" & `Type of pharmacological induction:[1]_I` == "apomorphine" ~ "Psychostimulant - APO", 
                                              CategoryDiseaseInduction == "Pharmacological (e.g. psychostimulants, NMDA antagonists)" & `Type of pharmacological induction:[1]_I` == "Other" ~ "Psychostimulant - Other", 
                                              CategoryDiseaseInduction == "Pharmacological (e.g. psychostimulants, NMDA antagonists)" & `Type of pharmacological induction:[1]_I` == "other" ~ "Psychostimulant - Other", 
                                              CategoryDiseaseInduction == "Pharmacological (e.g. psychostimulants, NMDA antagonists)" & `Type of pharmacological induction:[1]_I` == "amphetamine" ~ "Psychostimulant - Amphetamine", 
                                              CategoryDiseaseInduction == "Genetic (e.g. DISC1 KO, DAT KO, D2R overexpression)" & `Gene manipulated:[1]_I` == "DAT" ~ "Genetic - DAT KO", 
                                              CategoryDiseaseInduction == "Genetic (e.g. DISC1 KO, DAT KO, D2R overexpression)" & `Gene manipulated:[1]_I` == "DAT KO" ~ "Genetic - DAT KO", 
                                              TRUE ~ "Other"))

# Check if control group actually bigger than treatment to check direction                                              
...
diagnostic <- dataall %>% 
  filter(!is.na(F_S_m)) %>%
  mutate(ControlSham = abs(F_C_m - F_S_m), TreatmentSham = abs(F_T_m - F_S_m)) %>% 
  mutate(ControlCloser = ifelse(ControlSham < TreatmentSham, TRUE, FALSE )) %>% 
  mutate(result = ifelse(ControlCloser == FALSE & GreaterIsWorse_I == "TRUE", "TRUE", GreaterIsWorse_I), 
         result = ifelse(ControlCloser == TRUE & GreaterIsWorse_I == "TRUE", "FALSE", GreaterIsWorse_I)) %>%
  select(result) %>% 
  group_by(result) %>% 
  summarise(n=n())
...
# Replace unit of measurements for drugs with abbreviations (addressed higher)
# df <- df %>% 
...
df <- df %>% 
  ...
  mutate(DurationOfTreatmentWeeks = case_when(`Duration of treatment[1]_I` == 999 ~ NA_real_,
                                              ...
                                              `Unit of measurement for treatment duration[1]_I` == "Single dose" ~ 1/7))
...
df <- df %>% 
  mutate(TreatmentDurationCategory = case_when(DurationOfTreatmentWeeks <= 1 ~ "Less than 1 week", 
                                               ...
                                               is.na(DurationOfTreatmentWeeks) ~ "Not reported"))
...
df <- df %>% 
  rename(Species = `Species of animals?`, 
...
df <- df %>% 
  ...
  mutate(Efficacy = case_when(DrugName == "LK000764" ~ "TAAR1 full agonist", 
                              ...
                              DrugName == "Compound 6e" ~ "TAAR1 full agonist",
...
df <- df %>%
  rowwise() %>%
  mutate(RoBScoreAny = sum(c_across(contains("RoB")) == "Yes", na.rm = TRUE)) %>%
  ungroup() %>%
  mutate(RoBScore = paste0(RoBScoreAny, " criteria met"))

df$RoBScoreAny <- as.numeric(df$RoBScoreAny)

df$RoBTF <- 'No low RoB items'
for(i in 1:nrow(df)) {
  if(df[i, "RoBScoreAny"] > 0) {
...
### Function to clean up data frame columns

# The function detects columns where _I and _C suffixes are present (as well as the suffixless base columns), compares them numerically
# If they are the same (i.e., redundant), only the base/suffixless column is kept

# Generate unique base names from columns ending with _I or _C
base_names <- unique(sub("_[IC]$", "", names(df)))

# Create a new DataFrame where each unique base is processed
df_final <- reduce(base_names, function(data, base) {
  col <- base
  col_i <- paste0(base, "_I")
  col_c <- paste0(base, "_C")
  present_i <- col_i %in% names(data)
  present_c <- col_c %in% names(data)
  present_base <- col %in% names(data)
  
  # Adjust the equality check to correctly handle NAs
  if (present_i && present_c) {
    identical_i_c <- all(is.na(data[[col_i]]) & is.na(data[[col_c]]) | data[[col_i]] == data[[col_c]], na.rm = TRUE)
    
    if (identical_i_c) {
      if (present_base) {
        identical_all <- all(is.na(data[[col_i]]) & is.na(data[[col]]) | data[[col_i]] == data[[col]], na.rm = TRUE)
        
        if (identical_all) {
          # All three columns are effectively the same; drop _I and _C
          data <- select(data, -all_of(c(col_i, col_c)))
        } else {
          # Only _I and _C are identical, keep one, if no independent base exists, rename one
          data <- select(data, -all_of(col_c))
          if (!present_base) {
            data <- rename_with(data, .cols = all_of(col_i), .fn = ~ col)
          }
        }
      } else {
        # If no independent base column, just rename the remaining after dropping _C
        data <- select(data, -all_of(col_c))
        data <- rename_with(data, .cols = all_of(col_i), .fn = ~ col)
      }
    }
...
### Check columns with suffixes for similarities as a sense check
check <- df_final[,5:77]

# Custom sorting function for prioritizing _I and _C suffixes next to each other
ordered_cols <- names(check) %>%
  sort() %>%
  tibble::enframe(name = NULL, value = "name") %>%
  arrange(str_replace(name, "_[IC]$", ""), str_detect(name, "_[IC]$")) %>%
  pull(name)

# Reorder the DataFrame columns
check <- check %>% select(all_of(ordered_cols))
