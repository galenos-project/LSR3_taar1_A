...
1.  Flow of study selection and descriptives
    ...
    -   1.1 Table of included studies

    -   1.2 Description of experiment types and general approach
...
## given in side box for the 22 that got through TiAb screening
```
...
# process csv offline
# here, delete values in duplicate cells and replace with " to avoid kable giving NAs
tab2 <- read_csv("data/tab2.csv")
tab2$N <- as.numeric(tab2$N)

add_bold_lines <- function(df, rows) {
  # Add a class to the specified rows
  df$class <- ifelse(seq_len(nrow(df)) %in% rows, "bold-line", "")
  return(df)
}
```
...
## 1.2 Description of experiment types and methodological approach
...
#### 2.3.12b Alternative evaluation of ARRIVE reporting guidelines performance
...
#### 3.3.12b Alternative evaluation of ARRIVE reporting guidelines performance
...
The effect of administering a TAAR1 agonist on cognition in animals using NMD as the effect size is shown in Figure 3.4.2. The pooled estimate for NMD across all individual comparisons is displayed as a diamond shape at the bottom of the plot. Dotted lines indicate the prediction interval of the pooled estimate.
...
`r SMD_S_Cog_H2H[["k"]]` experimental comparisons were reported in `r SMD_S_Cog_H2H[["s.nlevels"]][3]` experiments reported from `r SMD_S_Cog_H2H[["s.nlevels"]][2]` publications and involving `r SMD_S_Cog_H2H[["s.nlevels"]][1]` different animal strains. The between strain variance was `r SMD_S_Cog_H2H$sigma2[1]`, the between study variance `r SMD_S_Cog_H2H$sigma2[2]` and the within-experiment variance `r SMD_S_Cog_H2H$sigma2[3]`.
...
`r SMD_S_LMA_TAA[["k"]]` experimental comparisons were reported in `r SMD_S_LMA_TAA[["s.nlevels"]][3]` experiments reported from `r SMD_S_LMA_TAA[["s.nlevels"]][2]` publications and involving `r SMD_S_LMA_TAA[["s.nlevels"]][1]` different animal strains. The between strain variance was `r SMD_S_LMA_TAA$sigma2[1]`, the between study variance `r SMD_S_LMA_TAA$sigma2[2]` and the within-experiment variance `r SMD_S_LMA_TAA$sigma2[3]`.
...
`r SMD_S_Cog_TAA[["k"]]` experimental comparisons were reported in `r SMD_S_Cog_TAA[["s.nlevels"]][3]` experiments reported from `r SMD_S_Cog_TAA[["s.nlevels"]][2]` publications and involving `r SMD_S_Cog_TAA[["s.nlevels"]][1]` different animal strains. The between strain variance was `r SMD_S_Cog_TAA$sigma2[1]`, the between study variance `r SMD_S_Cog_TAA$sigma2[2]` and the within-experiment variance `r SMD_S_Cog_TAA$sigma2[3]`.
...
`r SMD_S_LMA_KO[["k"]]` experimental comparisons were reported in `r SMD_S_LMA_KO[["s.nlevels"]][3]` experiments reported from `r SMD_S_LMA_KO[["s.nlevels"]][2]` publications and involving `r SMD_S_LMA_KO[["s.nlevels"]][1]` different animal strains.

Given the small number of studies, meta-analysis was not performed.
