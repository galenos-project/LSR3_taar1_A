...
-   Prepulse inhibition\*: `r length(table(df_S_PPI$StudyId))` studies and `r nrow(df_S_PPI)` comparisons in `r {strains <- unique(df_S_PPI$Strain); if (length(strains) > 1) paste(length(strains), "strains") else paste(length(strains), "strain")}`
    ...
    <!--# why the asterisk in the first 2? -->
...
Outcomes marked with asterisks (*) were primary outcomes
...
No studies were excluded on the basis of extreeme effect sizes reported.

## 4.1 Simple interventions

### 4.1.1 Outcome 1: Locomotor activity (a primary outcome)

The effect of administering a TAAR1 agonist on locomotor activity in animals using SMD as the effect size is shown in Figure 2. The pooled estimate (FT: and confidence interval?) for SMD across all individual comparisons is displayed as a diamond shape at the bottom of the plot. Dotted lines indicate the prediction interval of the pooled estimate.
...
<!--# No need to separate subgroup analyses and meta-regression as they are the same thing - they should be in the same section -->
...
<!--# these were not specified in the protocol, are they post-hoc because we could not run any low-risk RoB only sensitivity analysis? -->
...
Figure 5.1.2.9 displays the estimates for the pooled SMD's when comparisons are stratified by how many of the SYRCLE risk of bias assessment criteria (of which there are 10) that the experiment met. Whiskers indicate the 95% confidence interval of each estimate. The overall pooled SMD, not stratified by SYRCLE RoB assessment, is displayed as a diamond shape at the bottom of the plot.
...
Figure 5.1.2.10 displays the estimates for the pooled SMD's when comparisons are stratified by this categorisation that the experiment met. Whiskers indicate the 95% confidence interval of each estimate. The overall pooled SMD, not stratified by ARRIVE reporting quality guidelines, is displayed as a diamond shape at the bottom of the plot.
...
Figure 6.1.1.2.1 provides a visualisation of the meta-regression analysis predicting the response, measured as the Standardized Mean Difference (SMD), in locomotor activity to varying doses of SEP-363856 (in mg/kg).
...
Figure 6.1.1.2.2 provides a visualisation of the meta-regression analysis predicting the response, measured as the Standardized Mean Difference (SMD), in locomotor activity to varying doses of RO5203648 (in mg/kg).
...
Figure 6.1.1.2.3 provides a visualisation of the meta-regression analysis predicting the response, measured as the Standardized Mean Difference (SMD), in locomotor activity to varying doses of RO5166017 (in mg/kg).
...
Figure 6.1.1.2.4 provides a visualisation of the meta-regression analysis predicting the response, measured as the Standardized Mean Difference (SMD), in locomotor activity to varying doses of RO5263397 (in mg/kg).
...
Figure 6.1.1.2.5 provides a visualisation of the meta-regression analysis predicting the response, measured as the Standardized Mean Difference (SMD), in locomotor activity to varying doses of RO5256390 (in mg/kg).
