...
source("wrangling_u1/wrangling_functions.R", local = TRUE)
...
reconciled_cohort_label <- reconciled_records %>%
  mutate(
    Treatment1Type = case_when(
      str_detect(`InterventionLabel[1]`, regex("veh|con|saline", ignore_case = TRUE)) ~ "Negative control",
...
data <- data%>%
  mutate(drugname1 = case_when(
    ...
    grepl("ZH8651", data$`InterventionLabel[1]`) ~ "ZH8651",
    grepl("ZH8659", data$`InterventionLabel[1]`) ~ "ZH8659",
    grepl("ZH8667", data$`InterventionLabel[1]`) ~ "ZH8667",
    TRUE ~ "Other"
  ))
data <- data%>%
  mutate(drugname2 = case_when(
    ...
    grepl("ZH8651", data$`InterventionLabel[2]`) ~ "ZH8651",
    grepl("ZH8659", data$`InterventionLabel[2]`) ~ "ZH8659",
    grepl("ZH8667", data$`InterventionLabel[2]`) ~ "ZH8667",
    TRUE ~ "Other"
  ))
...
condition <- data$StudyId == 'dffa818e-98cd-4b2a-9d9a-9ed136e17669'
data[condition, columnnmae13] <- "C57Bl/6Jx129Sv/J (mouse)"
...
data[condition, columnnmae13] <- "Wistar (rat)"

condition <- data$StudyId == '5ec13944-5c99-413f-9070-795ea2a21e98'
data[condition, columnnmae13] <- "Not stated (mouse)"

condition <- (data$StudyId == 'fb8ed201-f663-48db-aae1-8ee88b355abd' & data$`Animal strain?` == "Other")
data[condition, columnnmae13] <- "C57Bl/6Jx129Sv/J (mouse)"
...
condition <- data$StudyId == "55bd4131-4668-401d-9e07-1935015bb592"
data[condition, columnname14] <- 'Shang P'
...
df <- df %>% 
  ...
  mutate(Efficacy = case_when(DrugName == "LK000764" ~ "TAAR1 full agonist", 
                              ...
                              DrugName == "ZH8651" ~ "TAAR1 full agonist",
                              DrugName == "ZH8667" ~ "TAAR1 full agonist",                             
                              TRUE ~ "NA")) %>% 
  ...
  mutate(MolarMass = case_when(DrugName == "LK000764" ~ 299.1058, 
                               ...
                               DrugName == "ZH8659" ~ 187.238,
                               DrugName == "ZH8651" ~ 200.08,
  ...
  mutate(EC50mM = case_when(DrugName == "LK000764" ~ 0.004, 
                            ...
                            DrugName == "Compound 6e" ~ 0.0465,
                            DrugName == "Compound 7b" ~ 0.00946,
                            DrugName == "Guanfacine" ~ 0.02,
                            DrugName == "ZH8651" ~ 0.00985,
                            DrugName == "ZH8667" ~ 0.00117,                           
                            TRUE ~ NA_real_)) 
...
df <- df %>% 
  ...
  mutate(StandardisedDose = log10((DoseOfIntervention_mgkg/1000)/((MolarMass)*(EC50mM/1000000)))) 
...
df <- df %>% 
  mutate(CDI_level2 = case_when(`Category of disease model induction method:[1]_I` == "Pharmacological (e.g. psychostimulants, NMDA antagonists)" & `Type of pharmacological induction:_I` == "cocaine" ~ "Stimulant or Dopamine Agonist", 
                                              `Category of disease model induction method:[1]_I` == "Pharmacological (e.g. psychostimulants, NMDA antagonists)" & `Type of pharmacological induction:_I` == "MK-801" ~ "NMDA antagonist",
                                              ...
                                              TRUE ~ `Category of disease model induction method:[1]_I`))  # Keep other values unchanged))

df <- df %>% 
  mutate(CDI_level3 = case_when(`Category of disease model induction method:[1]_I` == "Pharmacological (e.g. psychostimulants, NMDA antagonists)" & `Type of pharmacological induction:_I` == "cocaine" ~ "Cocaine", 
